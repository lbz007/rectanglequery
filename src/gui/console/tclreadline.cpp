 ...
 *  edi command support  
 */
static void        addOpenEDICommand(const char* cmd, const char* option);
static const char* getCommandOption(const char* cmd);
static int         registerEdiCommandList(Tcl_Interp* interp);
static void        setCommandOption(const char* cmd, const char* option);
static int         TclGetEdiCmdOption(ClientData cld, Tcl_Interp* itp, int argc,
                                      const char* argv[]);
enum {
    LINE_PENDING  = -1,
    LINE_EOF      = (1 << 8),
...
/**
 * edi command variables
 */
static std::map<std::string, std::string> edi_command_map;

static std::string edi_command_list = "";
...
/*                                MUTI-LINE SUPPORT FUNCTION AREA                               */
...
static int KeyRubout(int count, int key) {
    ...
    if (rl_end == 0 && rl_point == rl_end && (cur_index != muti_lines_size) && (cur_index)) {
...
/**
 * 
 * @param  {char*} cmd : all commands of edi with format {cmd1 cmd2 cmd3}
 */
static void addOpenEDICommand(const char* cmd, const char* option) {
    //
    edi_command_list += cmd;
    edi_command_list += " ";
    setCommandOption(cmd, option);
}

static int registerEdiCommandList(Tcl_Interp* interp) {
    int  status;
    auto pos = edi_command_list.rfind(" ");
    edi_command_list.erase(pos);
    static const char* cmd_list = edi_command_list.c_str();
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::openedi_cmd_list", (char*)&cmd_list, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "openedi_cmd_list", (char*)&cmd_list, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    return status;
}

static void setCommandOption(const char* cmd, const char* option) {
    if (edi_command_map.find(std::string(cmd)) == edi_command_map.end()) {
        edi_command_map[std::string(cmd)] = option;
    } else {
        printf("option of %s has been registered!\n", cmd);
    }
}

static const char* getCommandOption(const char* cmd) {
    if (edi_command_map.find(std::string(cmd)) != edi_command_map.end()) {
        auto option = edi_command_map[std::string(cmd)].c_str();
        return option;
    } else {
        printf("option of %s was not registered!\n", cmd);
        return nullptr;
    }
}

/*                                READ LINE FUNCTION AREA                                  */
static char* stripleft(char* in) {
    char* ptr = in;
    while (*ptr && *ptr <= ' ')
        ptr++;
...
static int TclReadlineCmd(ClientData clientData, Tcl_Interp* interp, int objc,
    ...
    enum SubCmdIdx {
        ...
        TCLRL_ADD,
        TCLRL_COMPLETE,
        TCLRL_CUSTOMCOMPLETER,
        TCLRL_BUILTINCOMPLETER,
        TCLRL_EOFCHAR,
    ...
    status = Tcl_GetIndexFromObj(interp, objv[1], subCmds, "option", 0, (int*)&obj_idx);
    ...
    case TCLRL_READ:
        ...
        rl_callback_handler_install(
          objc == 3 ? Tcl_GetStringFromObj(objv[2], 0) : "% ", TclReadlineLineCompleteHandler);
        if (first_time) {
            first_time = false;
            saveRawLineBuffer();
        }
    ...
    case TCLRL_CUSTOMCOMPLETER:
        if (objc > 3) {
            ...
            if (!blank_line(Tcl_GetStringFromObj(objv[2], 0)))
                tclrl_custom_completer = stripwhite(strdup(Tcl_GetStringFromObj(objv[2], 0)));
        }
    ...
    case TCLRL_BUILTINCOMPLETER:
        if (objc > 3) {
            ...
            if (TCL_OK != Tcl_GetBoolean(interp, Tcl_GetStringFromObj(objv[2], 0), &b_use)) {
    ...
    case TCLRL_EOFCHAR:
        if (objc > 3) {
            ...
            if (!blank_line(Tcl_GetStringFromObj(objv[2], 0)))
                tclrl_eof_string = stripwhite(strdup(Tcl_GetStringFromObj(objv[2], 0)));
        }
...
static int TclGetEdiCmdOption(ClientData cld, Tcl_Interp* interp, int argc,
                              const char* argv[]) {
    const char* option;
    if (2 == argc) {
        option = getCommandOption(argv[1]);
...
static void TclReadlineLineCompleteHandler(char* ptr) {
    ...
    if (!ptr) { /* <c-d> */
        ...
        if (status >= 2) {
            ...
            Tcl_AppendResult(tclrl_interp, "error in history expansion: ", expansion, "\n", (char*)NULL);
...
int Tclreadline_Init(Tcl_Interp* interp) {
    ...
    Tcl_CreateCommand(interp, "edi_proc_cmd", TclGetEdiCmdOption,
                      (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    tclrl_interp = interp;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::historyLength", (char*)&tclrl_history_length, TCL_LINK_INT)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::mutilineIndex", (char*)&muti_lines_size, TCL_LINK_INT)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::currentLineIndex", (char*)&cursor_index, TCL_LINK_INT)))
        return status;

    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::library", (char*)&tclrl_library, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::version", (char*)&tclrl_version_str, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::patchLevel", (char*)&tclrl_patchlevel_str, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::license", (char*)&tclrl_license, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;

    if (TCL_OK != (status = Tcl_LinkVar(interp, "tclreadline_library", (char*)&tclrl_library, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "tclreadline_version", (char*)&tclrl_version_str, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "tclreadline_patchLevel", (char*)&tclrl_patchlevel_str, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;
    if (TCL_OK != (status = Tcl_LinkVar(interp, "::tclreadline::historyPath", (char*)&tclrl_historypath_str, TCL_LINK_STRING | TCL_LINK_READ_ONLY)))
        return status;

    auto cmd_mgr = open_edi::infra::CommandManager::getCommandManager();

    auto cmds = cmd_mgr->getAllCommands();

    for (auto cmd : *cmds) {

        auto edi_cmd = cmd.first.c_str();
        auto options = cmd.second->getOptionsByOrder();
        ...
        auto option_list = str.c_str();
        addOpenEDICommand(edi_cmd, option_list);
    }

    // addOpenEDICommand("read_def", "-xxx -yyy -ooo");
    // addOpenEDICommand("read_test", "-xxx -yyy -ooo");

    status = registerEdiCommandList(interp);

    if (TCL_OK != status) return status;
...
static int TclReadlineInitialize(Tcl_Interp* interp, char* historyfile) {
    ...
    //     }
    // }
    // _rl_complete_show_unmodified = 1;
    _rl_complete_show_all = 1;
    return TCL_OK;
}
...
static char** TclReadlineCompletion(char* text, int start, int end) {
    ...
    if (text && ('!' == text[0] || (start && rl_line_buffer[start - 1] == '!' /* for '$' */))) {
        char* expansion = (char*)NULL;
