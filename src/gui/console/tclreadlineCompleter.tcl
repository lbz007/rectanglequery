...
namespace eval tclreadline {
    ...
    set edi_cmd_list_name [split $openedi_cmd_list]
    set edi_cmd_list_len  [llength $edi_cmd_list_name]

    if {$edi_cmd_list_len > 0} {
       for { set a 0}  {$a < $edi_cmd_list_len} {incr a} {
            proc complete([lindex $edi_cmd_list_name  $a]) {text start end line pos mod} {
                set cmd [Lindex $line 0]
                set cmd_dash [Lindex $line 1]
                switch -- $cmd_dash {
                    -  {
                        set ret [edi_proc_cmd $cmd]
                        set edi_option_list [split $ret]
                        return [CompleteFromList $text $edi_option_list]
                    }
                }
                return ""
            }
