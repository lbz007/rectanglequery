...
class PlaceDB (object):
    ...
    def get_cell_index(self, cell_name_array):
        ...
        arg_index = []
        for cell_nm in cell_name_array:

            arg_index.append(np.where(cell_nm == self.node_names)[0][0])
        return arg_index
    def get_cell_name(self, cell_index_array):

        arg_name = []
        for cell_index in cell_index_array:

            arg_name.append(self.node_names[cell_index])
        return arg_name
    ...
    def initialize_from_pydb(self, pydb, params):
        ...
        if pydb.num_routing_grids_x: 
            ...
            self.unit_vertical_capacities = np.array([params.unit_horizontal_capacity], dtype=self.dtype)
            self.initial_horizontal_demand_map = np.array(pydb.initial_horizontal_demand_map, dtype=self.dtype).reshape((-1, self.num_routing_grids_x, self.num_routing_grids_y)).sum(axis=0)
            self.initial_vertical_demand_map = np.array(pydb.initial_vertical_demand_map, dtype=self.dtype).reshape((-1, self.num_routing_grids_x, self.num_routing_grids_y)).sum(axis=0)
    ...
    def initialize(self, params):
        ...
        if params.scale_factor == 0.0 or self.site_width != 1.0:
            if params.scale_unit_flag: #, still relying on new scaling, as origin scale not working in lpdp.
