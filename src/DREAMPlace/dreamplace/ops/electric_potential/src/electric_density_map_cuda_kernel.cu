        ...
        )
{
    ...
    long long int i = (long long int)blockIdx.x * (long long int)blockDim.x + threadIdx.x;
    if (i < num_nodes)
    {
        // use stretched node size 
        T node_size_x = node_size_x_clamped_tensor[i];
    ...
    )
{
    ...
    if (i < (long long int)num_nodes * (long long int)num_impacted_bins_x * (long long int)num_impacted_bins_y)
    {
...
{
    int thread_count = 512;
    long long int block_count = ((long long int)num_nodes * (long long int)num_impacted_bins_x * (long long int)num_impacted_bins_y - 1 + thread_count) / thread_count;
