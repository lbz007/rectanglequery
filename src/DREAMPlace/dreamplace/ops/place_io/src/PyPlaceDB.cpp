...
PyPlaceDB::set(unsigned long db_ptr)
{
    ...
    if (db->getNumNets() > 0) {
      ...
      for (int i = 0; i < db->getNumNets(); ++i)
      {
        ...
        pybind11::list pins;
        int stopIdx = db->getFlatNet2PinStartMapV().at(i+1);
        for (int idx = db->getFlatNet2PinStartMapV().at(i); idx < stopIdx; ++idx)
        {
          pins.append(db->getFlatNet2PinMapV().at(idx));
        }
    ...
    for (int i = 0; i < db->getNumNodes(); ++i)
    {
      pybind11::list pins;
      int stopIdx = db->getFlatNode2PinStartMapV().at(i+1);
      for (int idx = db->getFlatNode2PinStartMapV().at(i); idx < stopIdx; ++idx)
      {
        pins.append(db->getFlatNode2PinMapV().at(idx));
      }
    ...
    for (int i = 0; i < db->getNumFences(); ++i)
    {
      pybind11::list boxes;
      int stopIdx = db->getFlatFenceBoxesStartV().at(i+1);
      for (int idx = db->getFlatFenceBoxesStartV().at(i); idx < stopIdx; ++idx)
      {
        boxes.append(db->getFlatFenceBoxesV().at(idx));
      }
      regions.append(boxes);
    }

    //TODO for routing info
    num_routing_grids_x = 0; ///< number of routing grids in x 
    num_routing_grids_y = 0; ///< number of routing grids in y 
    routing_grid_xl = 0; ///< routing grid region may be different from placement region 
    routing_grid_yl = 0; 
    routing_grid_xh = 0; 
    routing_grid_yh = 0;
    ...
    if (db->getNumNodes() > 0 && 
      ...
      {
        total_fixed_node_area += (double)(db->getNodeSizeXV().at(i)) * db->getNodeSizeYV().at(i);
