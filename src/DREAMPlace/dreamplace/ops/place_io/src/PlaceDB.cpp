...
void PlaceDB::add_def_row(DefParser::Row const& r) {
  ...
  /// choose smallest site height as core site.
  if (site.height() < siteHeight() && limbo::iequals(site.className(), "CORE")) {
    dreamplacePrint(kINFO, "set CORE site to %s, %d x %d\n",
                    site.name().c_str(), site.width(), site.height());
    m_coreSiteId = site.id();
  }
...
void PlaceDB::add_def_pin(DefParser::Pin const& p) {
  ...
  if (node.status() == PlaceStatusEnum::FIXED ||
      node.status() == PlaceStatusEnum::DUMMY_FIXED ||
      node.status() == PlaceStatusEnum::PLACED) {
      if (node.orient() == OrientEnum::N) {
          node.set(p.origin[0] + bbox[0], p.origin[1] + bbox[1],
                   p.origin[0] + bbox[2], p.origin[1] + bbox[3]);
      }
      else if (node.orient() == OrientEnum::S) {
          node.set(p.origin[0] - bbox[2], p.origin[1] - bbox[3],
                   p.origin[0] - bbox[0], p.origin[1] - bbox[1]);
      }
      else if (node.orient() == OrientEnum::E) {
          node.set(p.origin[0] + bbox[1], p.origin[1] - bbox[2],
                   p.origin[0] + bbox[3], p.origin[1] - bbox[0]);
      }
      else if (node.orient() == OrientEnum::W) {
          node.set(p.origin[0] - bbox[3], p.origin[1] + bbox[0],
                   p.origin[0] - bbox[1], p.origin[1] + bbox[2]);
      }
      node.setInitPos(ll(node));
  }
}
