    ...
    for (int i = 0; i < num_nets; ++i)
    {
        if (net_mask[i])
        {
            ...
            for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j)
            {
                ...
                grad_intermediate_x[flat_netpin[j]] = (a_x + b_x * xx) * exp_x - (a_nx + b_nx * xx) * exp_nx;

                T yy = y[flat_netpin[j]];
                T exp_y = exp((yy - y_max) * (*inv_gamma));
                T exp_ny = exp((y_min - yy) * (*inv_gamma));

                grad_intermediate_y[flat_netpin[j]] = (a_y + b_y * yy) * exp_y - (a_ny + b_ny * yy) * exp_ny;
            }
        }
    }
    ...
    int num_threads)
{
    ...
    DREAMPLACE_DISPATCH_FLOATING_TYPES(pos, "computeWeightedAverageWirelengthMergedLauncher", [&] {
