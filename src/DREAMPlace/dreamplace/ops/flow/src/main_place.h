...
/* Factory for whole place flow */
class PlaceFactory
{
  public:
    PlaceFactory(Para& para)
    {
      runPlacer(para);
    }
    ~PlaceFactory() = default;

    bool        runQPlace(Para& para)            const { return (para.getFlowSteps() & kQPlace);      }
    bool        runMPlace(Para& para)            const { return (para.getFlowSteps() & kMainPlace);   }
    void runPlacer(Para& para);
};
