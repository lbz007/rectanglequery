...
//void initDetailedPlaceDBGPU(DetailedPlaceDB<int>& db)
{
    ...
    {
       int *tmp;
       allocateCopyCUDA(tmp, getInitX(), num_nodes);
       db.init_x = tmp;
       allocateCopyCUDA(tmp, getInitY(), num_nodes);
       db.init_y = tmp;
       allocateCopyCUDA(tmp, getNodeSizeX(), num_nodes);
       db.node_size_x = tmp;
       allocateCopyCUDA(tmp, getNodeSizeY(), num_nodes);
       db.node_size_y = tmp;
       if (db.num_regions > 0)
       {
       ...
       allocateCopyCUDA(tmp, getNode2FenceMap(), num_nodes);  //refer to PyPlaceDB.cpp
       db.node2fence_region_map = tmp;
       allocateCopyCUDA(tmp, getCurX(), num_nodes);
       db.x = tmp;
       allocateCopyCUDA(tmp, getCurY(), num_nodes);
       db.y = tmp;
       ...
       allocateCopyCUDA(tmp, getFlatNode2PinStartMap(), num_nodes+1);
...
void CommonPlaceDB::updateXYGPU(const int* x, const int* y)
{
    int* cpu_dbx;
    int* cpu_dby;
    int num_nodes = getNumMoveableNodes();
