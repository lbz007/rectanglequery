...
typedef dbi::GeometryType PlGeoType;
...
inline PlDouble       dbuToMicrons(Plong i)              { return getTechLib()->dbuToMicrons(i); }
...
inline bool           isNetReset(PlNet* net)             { return (getNetType(net) == dbi::kNetTypeReset); }
inline PlDouble       getNetWeight(PlNet* net)           { return net->getWeight(); }
inline bool           isNetIgnored(PlNet* net)           { return (isNetClock(net) || isNetReset(net)); }
// term
...
inline PlSignalType   getTermType(PlTerm* term)          { return term->getType(); }
...
inline void           collectPinBoxes(PlPin* pin, BoxV& boxes)    { return pin->getBoxVector(boxes); }
...
inline bool           isInstMoveable(PlInst* inst)
{ 
  //from placement point of view, class pad or endcap is not movable, not matter its status is "fixed" or "placed".
  PlCell* cell = inst->getMaster();
  if (cell && (cell->isClassPad() || cell->isClassEndCap()) ) {
     return false;
  }
  return (isInstUnplaced(inst) || isInstPlaced(inst) || isInstSuggested(inst)); 
}
// cell
...
inline PlLGeometry*   getRegionLayerGeometry(PlConstraint* con) { return PlObj::addr<PlLGeometry>(con->getLayerGeometryId()); }
...
inline PlDouble       getBoxArea(PlBox& box)             { return (static_cast<PlDouble>(box.getHeight()) * box.getWidth()); }
...
#define forEachNetPins(net) \
    if (net != nullptr && getNetPinArray(net) != nullptr) {                                                           \
...
#define forEachInstPins(inst)                                                                            \
    if (inst != nullptr && getInstPinArray(inst) != nullptr) {                                                        \
...
#define forEachRegionBoxs(con)    \
    if (con != nullptr && getRegionLayerGeometry(con) != nullptr) {             \
        auto iter = getRegionLayerGeometry(con)->getBoxIter(); \
        for (PlBox *box = iter.getNext(); box != nullptr; box = iter.getNext()) {
