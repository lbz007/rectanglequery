...
const int kMainPlace      = (1 << 9) - 1;
const int kQPlace         = 1 << 9;
const int kFullPlace      = (1 << 10) - 1;

inline PlBox
getPin1Box(PlPin* pin)
{
  PlBox box;
  BoxV boxes;
...
class Para 
{
  public:
    ...
    {}
    Para(int part_num, int bins_x, int bins_y, int flows, bool saveDB, bool gpu, std::string file) :
    partition_num_(part_num), num_bins_x_(bins_x), num_bins_y_(bins_y), flow_steps_(flows),
  ...
  private:
    ...
    int    num_bins_x_     = 0;
    int    num_bins_y_     = 0;
    int    flow_steps_     = kFullPlace;   // default is to run LP and DP full flow
    int    num_threads_    = 8;
...
class CommonDB 
{
  public:
    ...
    void           setInitXV(LocV& xv)              { init_x_ = xv;                    }
    void           setInitYV(LocV& yv)              { init_y_ = yv;                    }
  private:
    bool __isDBLoaded();
    void __init();
    void __free();
...
class CommonPlaceDB
{
  ...
  public:
    ...
    int      getParaNumBinsX()          const { return getPara().getNumBinsX();        }
    int      getParaNumBinsY()          const { return getPara().getNumBinsY();        }
    bool     isGPU()                    const { return getPara().isGPU();              }
    int      numThreads()               const { return getPara().getNumThreads();      }
    int      getParaFlowSteps()         const { return getPara().getFlowSteps();       }
    ...
    const LocV&          getInitYV()                const { return db_.getInitY();                 }
    ...
    const IntV&          getNodeWeights()           const { return node_weights_;                  }
    ...
    PlOrient       getInstOriById(PlInt id)   const { return db_.getInstOrients().at(id);    }
    PlDirection    getPinDirById(PlInt id)    const { return db_.getPinDirections().at(id);  }
    // common DB interface : get ptr array
    const Coord*         getInitX()                 const { return db_.getInitX().data();                 }
    const Coord*         getInitY()                 const { return db_.getInitY().data();                 }
    ...
    const PlInt*         getNodeWeight()            const { return node_weights_.data();                  }
    ...
    PlInt        getNodeWeight(int id)            const { return node_weights_.at(id); }
    ...
    void     setNodeWeight(const int id, const PlInt w)  { node_weights_[id] = w;             }
    ...
    void     copyCur2InitLoc()
             {
               db_.setInitXV(cur_x_);
               db_.setInitYV(cur_y_);
             }

__attribute__ ((visibility("default")))
    void     updateXY(const int* x, const int* y);
    //  detailedDB interface
    void     initDetailedPlaceDB(DetailedPlaceDB<int>& db, bool isGS=false);
    ...
    void     printCurLocations(int num);
    void     reportHPWL(const String& stage);
    void     reportNetStat(); 
    void     reportFLUTE(const String& stage);
    PlInt    calcNetHPWL(int netIdx);
    std::pair<PlInt, PlInt> calcNetFLUTE(int netIdx);
#ifdef _CUDA_FOUND
  ...
  private:
    ...
    CommonDB            db_;                               // edi db
    IntV                node_weights_;                     // weight of nodes, for LP only
    ...
    int                 num_bins_x_         = 1024;        // num of bins in x
    int                 num_bins_y_         = 1024;        // num of bins in y
