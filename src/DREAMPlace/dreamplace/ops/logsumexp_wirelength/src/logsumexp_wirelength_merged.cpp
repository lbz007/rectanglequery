    ...
    for (int i = 0; i < num_nets; ++i)
    {
        if (net_mask[i])
        {
            ...
            partial_wl[i] = (log(exp_x_sum) + log(exp_nx_sum)) * (*gamma) + x_max - x_min 
                + (log(exp_y_sum) + log(exp_ny_sum)) * (*gamma) + y_max - y_min;
            ...
            for (int j = netpin_start[i]; j < netpin_start[i+1]; ++j)
            {
                ...
                T exp_x = exp((xx - x_max) * (*inv_gamma));
                T exp_nx = exp((x_min - xx) * (*inv_gamma));
                grad_intermediate_x[jj] = (exp_x*reciprocal_exp_x_sum - exp_nx*reciprocal_exp_nx_sum); 

                T yy = y[jj];
                T exp_y = exp((yy - y_max) * (*inv_gamma));
                T exp_ny = exp((y_min - yy) * (*inv_gamma));
                grad_intermediate_y[jj] = (exp_y*reciprocal_exp_y_sum - exp_ny*reciprocal_exp_ny_sum); 
            }
        }
    }
}
        ...
        ) 
{
    ...
    DREAMPLACE_DISPATCH_FLOATING_TYPES(pos, "computeLogSumExpWirelengthLauncher", [&] {
