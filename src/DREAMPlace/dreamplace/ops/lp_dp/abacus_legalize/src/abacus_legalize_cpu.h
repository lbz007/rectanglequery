...
#ifndef DREAMPLACE_OEM_ABACUS_LEGALIZE_CPU_H
#define DREAMPLACE_OEM_ABACUS_LEGALIZE_CPU_H
...
struct AbacusCluster
{
    ...
    Plong e; ///< weight of displacement in the objective
    Plong q; ///< x = q/e
    T w; ///< width 
        ...
        )
{
    for (int i = 0; i < num_nodes; i += 1) 
    {
        ...
        else // fixed nodes may distribute to multiple bins  
        {
            ...
            int bin_id_xh = std::min((int)ceil(static_cast<double>(x[node_id]+node_size_x[node_id]-xl)/bin_size_x), num_bins_x);
            int bin_id_yl = std::max((y[node_id]-yl)/bin_size_y, (T)0);
            int bin_id_yh = std::min((int)ceil(static_cast<double>(y[node_id]+node_size_y[node_id]-yl)/bin_size_y), num_bins_y);
...
{
    // a very large number 
    Plong M = pow(10, ceil(static_cast<double>(log(static_cast<Plong>(xh-xl)*num_row_nodes))/log(10)));
        ...
        )
{
    for (unsigned int i = 0; i < bin_cells.size(); i += 1)
    {
        ...
        if (!row2nodes.empty())
        {
            removeContainedFixedCellsFromRow(row2nodes, num_movable_nodes, x, node_size_x);
    ...
    double average = static_cast<double>(displace)/num_movable_nodes;
    ...
    for (auto const& cells : bin_cells)
    {
        ...
        for (auto node_id : cells)
        {
            if (node_id < num_movable_nodes)
            {
                ...
                xxl += ceil(static_cast<double>(node_size_x[node_id])/site_width)*site_width; 
            }
            else if (node_id < num_nodes)
            {
                xxl = ceil(static_cast<double>(x[node_id]+node_size_x[node_id]-xl)/site_width)*site_width+xl; 
            }
        }
    }
    //align2SiteCPU(
