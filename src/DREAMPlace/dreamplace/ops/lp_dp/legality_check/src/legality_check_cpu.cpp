     ...
     )
{
  ...
  if (!boundaryCheckNew(x, y, node_size_x, node_size_y, xl, yl, xh, yh, num_movable_nodes)) {
    legal_flag = false; 
  }

  // check row and site alignment 
  if (!siteAlignmentCheckNew(
...
bool legalityCheckRun(CommonPlaceDB* db)
{
  ...
  bool legal_flag = legalityCheckCPU(
                    ...
                    db->getNumNodes() - db->getNumIOPins(),   //< movable and fixed cells, no io 
                    db->getNumMoveableNodes(), db->getNumFences()
                    );

  return ((legal_flag) ? 0 : 1); 
}
