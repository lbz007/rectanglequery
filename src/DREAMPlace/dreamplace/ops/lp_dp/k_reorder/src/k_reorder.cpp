    ...
    for (int i = 0; i < db.num_sites_y; ++i)
    {
        for (unsigned int j = 0; j < state.row2node_map.at(i).size(); ++j)
        {
            ...
            if (node_id < db.num_movable_nodes)
            {
                ...
                dreamplaceAssertMsg(space >= db.node_size_x[node_id], "space %d, node_size_x[%d] %d, original space (%d, %d), site_width %d", space, node_id, db.node_size_x[node_id], space_xl, space_xh, db.site_width);
            }
#ifdef DEBUG
            if (node_id < db.num_movable_nodes)
            {
...
//pragma omp parallel for num_threads (state.num_threads) schedule(dynamic, 1)
    ...
    {
        auto time = std::accumulate(compute_reorder_hpwl_time, compute_reorder_hpwl_time+MAX_NUM_THREADS, (float)0);
    ...
    {
        auto time = std::accumulate(apply_reorder_time, apply_reorder_time+MAX_NUM_THREADS, (float)0); 
