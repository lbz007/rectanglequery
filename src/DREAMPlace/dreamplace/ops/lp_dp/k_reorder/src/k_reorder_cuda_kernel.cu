...
struct KReorderStateCU 
{
    ...
    int* instance_nets_size; ///< reorder_instances.size2, number of nets for each instance 
    ...
    int64_t* net_hpwls; ///< used for compute HPWL 
};
        ...
        )
{
    ...
    {
        printf("idx_bgn %d, idx_end %d, row size %u\n", inst.idx_bgn, inst.idx_end, state.row2node_map.size(inst.row_id));
    }
...
template <typename DetailedPlaceDBType, typename StateType>
inline __device__
typename DetailedPlaceDBType::type compute_instance_hpwl(
        const DetailedPlaceDBType& db, 
        const StateType& state, 
        const KReorderInstance& inst, 
        int permute_id, 
...
__global__ void reduce_min_2d_cub(const T* __restrict__ costs, int* best_permute_id, int m, int n)
{
    ...
    if (threadIdx.x == 0)
    {
        //printf("inst[%d] cost %g, permute_id %d\n", blockIdx.x, aggregate.value, aggregate.index);
...
template <typename T>
__global__ void compute_node2inst_map(DetailedPlaceDB<T> db, KReorderStateCU<T> state, int group_id, int offset)
{
    __shared__ int group_size; 
    if (threadIdx.x == 0)
    {
...
template <typename T>
__global__ void compute_net_markers(DetailedPlaceDB<T> db, KReorderStateCU<T> state)
{
...
template <typename T>
__global__ void print_net_markers(DetailedPlaceDB<T> db, KReorderStateCU<T> state)
{
...
template <typename T>
__global__ void compute_instance_nets(DetailedPlaceDB<T> db, KReorderStateCU<T> state, int group_id, int offset)
{
    __shared__ int group_size; 
    if (threadIdx.x == 0)
    {
...
template <typename T>
__global__ void compute_instance_nets(DetailedPlaceDB<T> db, KReorderStateCU<T> state)
{
    for (int net_id = blockIdx.x * blockDim.x + threadIdx.x; net_id < db.num_nets; net_id += blockDim.x * gridDim.x)
    {
        if (state.net_markers[net_id])
        {
            int net2pin_id = db.flat_net2pin_start_map[net_id];
...
template <typename T>
__global__ void unique_instance_nets(DetailedPlaceDB<T> db, KReorderStateCU<T> state, int group_id)
{
    __shared__ int group_size; 
    if (threadIdx.x == 0)
    {
...
__global__ void reset_state(DetailedPlaceDBType db, StateType state, int group_id)
{
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < state.reorder_instances.size2; i += blockDim.x * gridDim.x)
    {
...
template <typename T>
#ifdef DYNAMIC
__global__ void k_reorder(DetailedPlaceDB<T> db, KReorderStateCU<T> state)
#else 
void k_reorder(DetailedPlaceDB<T>& db, KReorderStateCU<T>& state, const std::vector<std::vector<KReorderInstance> >& host_reorder_instances)
#endif
{
#ifdef TIMER 
    hr_clock_rep timer_start, timer_stop; 
    hr_clock_rep enumeration_time = 0, apply_reorder_time = 0; 
...
#else 
        dreamplaceAssert(state.reorder_instances.size1 == host_reorder_instances.size());
...
#ifdef TIMER
    dreamplacePrint(kDEBUG, "enumeration takes %g ms for %d runs, average %g ms\n", get_timer_period()*enumeration_time, enumeration_runs, get_timer_period()*enumeration_time/enumeration_runs);
    dreamplacePrint(kDEBUG, "apply_reorder takes %g ms for %d runs, average %g ms\n", get_timer_period()*apply_reorder_time, apply_reorder_runs, get_timer_period()*apply_reorder_time/apply_reorder_runs);
#endif 
}
...
int kreorderCUDALauncherNew(DetailedPlaceDB<T>& db, int K, int max_iters, int num_threads)
{
    ...
    // initialize cpu db from db 
    {
        ...
        dreamplacePrint(kDEBUG, "initializing CPU DB takes %g ms\n", get_timer_period()*(iter_time_stop-iter_time_start));

        iter_time_start = get_globaltime(); 
        compute_row_conflict_graph(cpu_db, host_row2node_map, host_adjacency_matrix, host_row_graph, num_threads); 
        compute_independent_rows(cpu_db, host_row_graph, host_independent_rows); 
        iter_time_stop = get_globaltime(); 
        dreamplacePrint(kDEBUG, "computing independent rows takes %g ms\n", get_timer_period()*(iter_time_stop-iter_time_start));

        iter_time_start = get_globaltime(); 
        compute_reorder_instances(cpu_db, host_row2node_map, host_independent_rows, host_reorder_instances, state.K);
        iter_time_stop = get_globaltime(); 
        dreamplacePrint(kDEBUG, "computing reorder instances takes %g ms\n", get_timer_period()*(iter_time_stop-iter_time_start));
    }
    ...
    iter_time_start = get_globaltime(); 
    {
        ...
        allocateCUDA(state.net_hpwls, db.num_nets, int64_t);
    }
    iter_time_stop = get_globaltime(); 
    dreamplacePrint(kDEBUG, "initializing CUDA state takes %g ms\n", get_timer_period()*(iter_time_stop-iter_time_start));
    ...
    iter_time_stop = get_globaltime(); 
    dreamplacePrint(kDEBUG, "destroying CUDA state takes %g ms\n", get_timer_period()*(iter_time_stop-iter_time_start));
    ...
    iter_time_stop = get_globaltime(); 
    dreamplacePrint(kDEBUG, "destroying CPU state takes %g ms\n", get_timer_period()*(iter_time_stop-iter_time_start));
...
#define REGISTER_KERNEL_LAUNCHER(T) \
    void instantiateKReorderCUDALauncher(\
