...
#ifndef DREAMPLACE_OEM_BIN_ASSIGNMENT_H
#define DREAMPLACE_OEM_BIN_ASSIGNMENT_H
...
template <typename T>
void distributeCells2BinsCPUNew(
...
template <typename T>
void distributeFixedCells2BinsCPUNew(
        const LegalizationDB<T>& db, 
        const T* x, const T* y, 
        const T* node_size_x, const T* node_size_y, 
        T bin_size_x, T bin_size_y, 
...
template <typename T>
void distributeBlanks2BinsCPUNew(
        const T* x, const T* y, 
        const T* node_size_x, const T* node_size_y, 
            ...
            for (unsigned int bi = 0; bi < blanks.size(); ++bi)
            {
                ...
                for (unsigned int ci = 0; ci < cells.size(); ++ci)
                {
                    ...
                    if (node_yh > blank.yl && node_yl < blank.yh && node_xh > blank.xl && node_xl < blank.xh) // overlap 
                    {
                        ...
                        {
                            blank.xl = ceil(static_cast<double>(node_xh-xl)/site_width)*site_width+xl; // align blanks to sites
                        }
        ...
        )
{
    for (int i = 0; i < num_bins_x*num_bins_y; i += 1) 
    {
        ...
        long long capacity = static_cast<long long>(bin_xh-bin_xl)*(bin_yh-bin_yl);
        ...
        bin_capacities[i] = ceil(static_cast<double>(capacity) / (site_width*row_height)); 
    }
}
