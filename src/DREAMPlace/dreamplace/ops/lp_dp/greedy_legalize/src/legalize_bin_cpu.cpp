        ...
        ) 
{
    for (int i = 0; i < num_bins_x*num_bins_y; i += 1) 
    {
        ...
        int blank_num_bins_per_bin = round(static_cast<double>(bin_size_y)/blank_bin_size_y);
        ...
        if (lr_flag)
        {
            std::sort(cells.begin(), cells.end(), CompareByNodeNTUPlaceCostFromLeftCPUNew<T>(init_x, init_y, node_size_x, node_size_y));
        }
        else 
        {
            std::sort(cells.begin(), cells.end(), CompareByNodeNTUPlaceCostCPUNew<T>(init_x, init_y, node_size_x, node_size_y));
        }
        ...
        for (int ci = bin_cells.at(i).size()-1; ci >= 0; --ci)
        {
            ...
            double init_xl = floor(((alpha*init_x[node_id]+(1-alpha)*x[node_id])-xl)/site_width)*site_width+xl;
            double init_yl = (alpha*init_y[node_id]+(1-alpha)*y[node_id]);
            T width = ceil(static_cast<double>(node_size_x[node_id])/site_width)*site_width;
            T height = node_size_y[node_id];


            int num_node_rows = ceil(static_cast<double>(height)/row_height); // may take multiple rows 
            ...
            for (int bin_id_offset_y = 0; abs(bin_id_offset_y) < blank_bin_id_dist_y; bin_id_offset_y = (bin_id_offset_y > 0)? -bin_id_offset_y : -(bin_id_offset_y-1))
            {
                ...
                double row_best_cost = xh-xl+yh-yl;
                double row_best_xl = -1;
                double row_best_yl = -1;
                ...
                for (unsigned int bi = 0; search_flag && bi < bin_blanks.at(blank_bin_id).size(); ++bi)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (intersect_blank_width >= width)
                        {
                            ...
                            double cost = fabs(target_xl-init_xl)+fabs(target_yl-init_yl);
                            // update best cost 
            ...
            if (best_blank_bin_id_y >= 0)
            {
              ...
              #ifdef DEBUG 
              dreamplacePrint(kDEBUG, "cell: %d, node_id: %d, not find proper blanks in bin: %d.\n", ci, node_id, i);
              #endif
            }
        }
        ...
        ) 
{
    for (int i = 0; i < num_bins_x*num_bins_y; i += 1) 
    {
        ...
        std::sort(cells.begin(), cells.end(), CompareByNodeNTUPlaceCostBySizeFromLeftCPUNew<T>(init_x, init_y, node_size_x, node_size_y));

        // from large to small, from left to right.
        for (int ci = bin_cells.at(i).size()-1; ci >= 0; --ci)
        {
            int node_id = cells.at(ci); 
            // align to site 
            ...
            for (int bin_id_offset_y = 0; abs(bin_id_offset_y) < blank_bin_id_dist_y; bin_id_offset_y = (bin_id_offset_y > 0)? -bin_id_offset_y : -(bin_id_offset_y-1))
            {
                ...
                // blanks in this bin 
                const std::vector<Blank<T> >& blanks = bin_blanks.at(blank_bin_id);
                const int blsize = blanks.size();
                if (blsize == 0) {
                  continue;
                }
                int target_bid = blsize - 1;
                for (int i = 0 ; i < blsize; ++i)
                {
                  if (blanks.at(i).xl >= init_xl) {
                     target_bid = i;
                     break;
                  }
                }
                ...
                for (int bi = target_bid; search_flag && bi >= 0 ; --bi)
                {
                    const Blank<T>& blank = blanks.at(bi);
                ...
                search_flag = true;
                for (int bi = target_bid+1 ; search_flag && bi < blsize; ++bi)
                {
                    const Blank<T>& blank = blanks.at(bi);
            ...
            if (best_blank_bin_id_y >= 0)
            {
              ...
              #ifdef DEBUG
              dreamplacePrint(kINFO, "cell: %d, node_id: %d, not find proper blanks in bin: %d.\n", ci, node_id, i);
              #endif
            }
        }
        *num_unplaced_cells += bin_cells.at(i).size();
...
void instantiatelegalizeBinCPUNew(
...
void instantiatelegalizeBinCPUNew(
        ...
        const int* node_size_x, const int* node_size_y, 
        std::vector<std::vector<Blank<int> > >& bin_blanks, // blanks in each bin, sorted from low to high, left to right 
        std::vector<std::vector<int> >& bin_cells, // unplaced cells in each bin 
        int* x, int* y, 
        int num_bins_x, int num_bins_y, int blank_num_bins_y, 
        int bin_size_x, int bin_size_y, int blank_bin_size_y, 
        int site_width, int row_height, 
...
void instantiatelegalizeBinBySizeCPUNew(
        const float* init_x, const float* init_y, 
        const float* node_size_x, const float* node_size_y, 
        std::vector<std::vector<Blank<float> > >& bin_blanks, // blanks in each bin, sorted from low to high, left to right 
        std::vector<std::vector<int> >& bin_cells, // unplaced cells in each bin 
