...
class Tech : public Object {
  public:
    ...
    ObjectIndex getLayerLEFIndexByName(const char *layer_name);
    ...
    ObjectId getPropertyDefinitionVectorId();
    ObjectId getPropertyDefinitionId(const char *prop_name);
    PropertyDefinition* getPropertyDefinition(const char *prop_name);
    ...
    ArrayObject<ObjectId> *getLayerArray() const;

    ViaMaster *createAndAddViaMaster(std::string &name);
    ViaRule *createViaRule(std::string &name);

    ViaMaster *getViaMaster(const std::string &name);
    ViaRule *getViaRule(const std::string &name) const;

    int getViaMasterIndex(ViaMaster *via_master);
    ViaMaster *getViaMasterByIndex(int via_master_index);
    ...
    int64_t micronsToDBU(double microns);
    ...
    // Hier owner index:
    ObjectIndex getHierOwnerIndex() const { return hier_owner_index_; }
    void setHierOwnerIndex(ObjectIndex index) { hier_owner_index_ = index;}
  private:
    // first 32 bits
    ...
    ObjectId layer_ids_;
    ObjectId sites_;
    ObjectId property_definitions_array_;
