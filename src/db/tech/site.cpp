...
void SitePatternPair::printLEF(IOManager &io_manager) const {
    io_manager.write("%s %s ", getName(), toString(orientation_).c_str());
}
...
void Site::setPatternSize(UInt32 v) {
    if (v == 0) {
        if (site_patterns_) {
            __destroyObjectIdArray(site_patterns_);
        }
        return;
    }
    if (!site_patterns_) {
...
void Site::printLEF(IOManager &io_manager) const {
    io_manager.write("SITE %s\n", getName());
    io_manager.write("   CLASS %s ;\n", toString(class_).c_str());
    if (numSitePatterns() > 0) {
        io_manager.write("   ROWPATTERN ");
        IdArray *vobj =
            addr<IdArray>(site_patterns_);
        ...
        io_manager.write("   SYMMETRY %s ;\n", toString(symmetry_).c_str());
    }
    Tech *lib = getTechLib();
    io_manager.write("   SIZE %g BY %g ;\n",
        lib->dbuToMicrons(width_), lib->dbuToMicrons(height_));
