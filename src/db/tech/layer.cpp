...
void AntennaModel::addDiffAreaRatioPWL(float d, float r) {
    ...
    if (diff_area_ratio_pwl_ == 0) {
        vct = Object::createObject<ArrayObject<FloatPair>>(
...
void CurrentDen::reset() {
    memset((uint8_t*)this + sizeof(Object), 0, sizeof(CurrentDen) - sizeof(Object));
}
...
int CurrentDen::getFrequencySize() const {
    //return frequencies_;
    if (frequencies_ != 0) {
        ArrayObject<float> *array = addr<ArrayObject<float>>(frequencies_);
        if (array)
...
void CurrentDen::setFrequecies(float* frequencies, UInt32 freq_num) {
    ...
    if (frequencies_ == 0) {
        array_ptr = Object::createObject<ArrayObject<float>>(
                      kObjectTypeArray, lib_id);
...
int CurrentDen::getWidthSize() const {
    if (widths_ != 0) {
        ArrayObject<UInt32> *array = addr<ArrayObject<UInt32>>(widths_);
        if (array)
        return array->getSize();
...
int CurrentDen::getCutAreaSize() const {
    if (cut_areas_ != 0) {
        ArrayObject<float> *array = addr<ArrayObject<float>>(cut_areas_);
        if (array)
            return array->getSize();
...
ACCurrentDen* CurrentDenContainer::getInitACPeak() {
    ...
    if (current_dens_ == 0) {
        ...
        for (int i = 0; i < kCurrentMax; i++) {
            CurrentDen *den = Object::createObject<CurrentDen>(
                                  kObjectTypeCurrentDen, lib_id);
            if (vct)
...
void MinArea::appendExceptMinSizePair(UInt32 min_width, UInt32 min_len) {
    ...
    if (except_min_size_ == 0) {
        vct = Object::createObject<ArrayObject<UintPair>>(
                kObjectTypeArray, lib_id);
        if (vct == nullptr) return;
...
AntennaModel* Layer::getInitAntennaModel(UInt32 index) {
    ...
    if (antenna_ == 0) {
        ...
        {
            AntennaModel *am = Object::createObject<AntennaModel>(
                                kObjectTypeAntennaModel, lib_id);
