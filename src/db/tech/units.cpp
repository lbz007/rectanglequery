...
void Units::printLEF(IOManager &io_manager) const {
    io_manager.write("UNITS\n");
    if (time_unit_ != kInvalidSymbolIndex)
        io_manager.write("   TIME %s %d ;\n",
            getTimeUnits(), time_factor_);
    if (capacitance_unit_ != kInvalidSymbolIndex)
        io_manager.write("   CAPACITANCE %s %d ;\n",
            getCapacitanceUnits(), capacitance_factor_);
    if (resistance_unit_ != kInvalidSymbolIndex)
        io_manager.write("   RESISTANCE %s %d ;\n",
            getResistanceUnits(), resistance_factor_);
    if (power_unit_ != kInvalidSymbolIndex)
        io_manager.write("   POWER %s %d ;\n",
            getPowerUnits(), power_factor_);
    if (current_unit_ != kInvalidSymbolIndex)
        io_manager.write("   CURRENT %s %d ;\n",
            getCurrentUnits(), current_factor_);
    if (voltage_unit_ != kInvalidSymbolIndex)
        io_manager.write("   VOLTAGE %s %d ;\n",
            getVoltageUnits(), voltage_factor_);
    if (length_unit_ != kInvalidSymbolIndex)
        io_manager.write("   DATABASE %s %d ;\n",
            getLengthUnits(), length_factor_);
    if (frequency_unit_ != kInvalidSymbolIndex)
        io_manager.write("   FREQUENCY %s %d ;\n",
            getFrequencyUnits(), frequency_factor_);
    io_manager.write("END UNITS\n\n");
}
