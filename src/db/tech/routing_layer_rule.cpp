...
int CornerSpacing::getWidthSize() const{
    if (widths_ != 0) {
        ArrayObject<Int32> *width_array = addr<ArrayObject<Int32>>(widths_);
        if (width_array)
        return width_array->getSize();
    } 
    return 0;
}
...
int CornerSpacing::getHorizontalSpacingSize() const{
    if (horizontal_spacings_ != 0) {
        ArrayObject<Int32> *hori_array = addr<ArrayObject<Int32>>(horizontal_spacings_);
        if (hori_array)
            return hori_array->getSize();
    }
    return 0;
}
...
int CornerSpacing::getVerticalSpacingSize() const{
    if (vertical_spacings_ != 0) {
        ArrayObject<Int32> *vetical_array = addr<ArrayObject<Int32>>(vertical_spacings_);
        if (vetical_array)
            return vetical_array->getSize();
    }
    return 0;
}
...
int DirSpanLengthSpTbl::ExactSLSpacing::getSpacingSize() const { 
    if (exact_spacings_ != 0) {
        ArrayObject<Int32> *spacing_array = addr<ArrayObject<Int32>>(exact_spacings_);
        if (spacing_array)
            return spacing_array->getSize();
    }
    return 0;
}
...
UInt32 DirSpanLengthSpTbl::SpanLength::getSpacing(UInt32 idx) const {
 ...
 int DirSpanLengthSpTbl::SpanLength::getSpacingSize() const {
    if (row_spacings_ != 0) {
        ArrayObject<Int32> *spacing_array = addr<ArrayObject<Int32>>(row_spacings_);
        if (spacing_array)
            return spacing_array->getSize();
...
int DirSpanLengthSpTbl::getPRLSize() const {
    if (prls_ != 0) {
        ArrayObject<UInt32> *array = addr<ArrayObject<UInt32>>(prls_);
        if (array)
...
void DirSpanLengthSpTbl::addPRL(UInt32 prl) {
    ...
    if (prls_ == 0) {
        array_ptr = Object::createObject<ArrayObject<UInt32>>(
                        kObjectTypeArray, lib_id);
        if (array_ptr == nullptr) return;
        array_ptr->setPool(getStorageUtil()->getPool());
        array_ptr->reserve(16);        
...
void DirSpanLengthSpTbl::setNumSpanLengths(UInt32 num) {
    ...
    while(num != 0) {
        num--;
        DirSpanLengthSpTbl::SpanLength *sl = Object::createObject<
                DirSpanLengthSpTbl::SpanLength>(kObjectTypeSpanLength, lib_id);
        if (vct)
...
int DirSpanLengthSpTbl::getExactSLSpacingListSize() const {
    if (exact_sl_spacings_ != 0) {
        ArrayObject<ObjectId> *exact_sl_spacings = addr<ArrayObject<ObjectId>>(exact_sl_spacings_);
        if (exact_sl_spacings)
            return exact_sl_spacings->getSize();
    }
    return 0;
}
...
EOLKeepout::EOLKeepout() {
    memset((uint8_t*)this + sizeof(Object), 0, sizeof(EOLKeepout) - sizeof(Object));
    ArrayObject<SymbolIndex> *vct = nullptr;

    if (class_names_ == 0) {
        vct = Object::createObject<ArrayObject<SymbolIndex>>(
                kObjectTypeArray, getTechLib()->getId());
        if (vct == nullptr)
            return;
        ...
        vct->reserve(16);
        if (vct) {
            vct->pushBack(getTechLib()->getOrCreateSymbol("default_noclass"));
            class_id_ = 0;
        }
    }
}
...
const char* EOLKeepout::getClassName() {
    ...
    if (vct) {
        ...
        if (sym_index != kInvalidSymbolIndex) {
            return getTechLib()->getSymbolByIndex(sym_index).c_str();
        }
    }
    return nullptr;
}
    ...
    if (vct) {
        if (name || name[0] != '\0') {
            for (int i = 0; i < vct->getSize(); ++i) {
              std::string &class_name = getTechLib()->getSymbolByIndex((*vct)[i]);
                if (strcmp(class_name.c_str(), name) == 0) {
                    class_id_ = i;
                    return;
                }
            }

            vct->pushBack(getTechLib()->getOrCreateSymbol(name));
 ...
 * @return
 */
ArrayObject<ObjectId>* MinCut::getCutClassArr() const {
    if (cut_classes_ != 0) {
...
int MinCut::getCutClassSize() const {
    if (cut_classes_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(cut_classes_);
        if (array)
            return array->getSize();
 ...
 * @return
 */
 ArrayObject<UInt32>* MinCut::getCutClassNumCutsArr() const {
    if (cc_num_cuts_ != 0) {
...
int MinCut::getCutClassNumCutsSize() const {
    if (cc_num_cuts_ != 0) {
        ArrayObject<UInt32> *array = addr<ArrayObject<UInt32>>(cc_num_cuts_);
        if (array)
            return array->getSize();
 ...
 * @return
 */
ArrayObject<ObjectId>* ProtrusionRule::getProtrusionWidths() const {
    if (protrusion_width_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(protrusion_width_);
        return array;
    } else {
...
int ProtrusionRule::getProtrusionWidthSize() const {
    if (protrusion_width_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(protrusion_width_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getEOLKeepoutSize() const {
    //return eol_keepout_list_;
    if (eol_keepouts_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(eol_keepouts_);
        if (array)
...
int RoutingLayerRule::getBoundaryEOLBlocakgeSize() const {
    if (boundary_eol_blks_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(boundary_eol_blks_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getCornerEOLKeepoutSize() const {
     if (corner_eol_keepouts_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(corner_eol_keepouts_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getCornerFillSpacingSize() const {
     if (corner_fill_spacings_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(corner_fill_spacings_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getCornerSpacingSize() const {
    if (corner_spacings_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(corner_spacings_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getDirSpanLengthSpTblSize() const {
    if (dsls_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(dsls_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getMinCutSize() const {
    if (min_cuts_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(min_cuts_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getMinEnclAreaSize() const {
    if (min_enclosed_areas_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(min_enclosed_areas_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getMinSizeSize() const {
    //return min_size_;
    if (min_sizes_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(min_sizes_);
        if (array)
...
int RoutingLayerRule::getMinStepSize() const {
   if (min_steps_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(min_steps_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getProtrusionRuleSize() const {
    if (protrusions_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(protrusions_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getWidthSpTblSize() const {
    if (width_sp_tbls_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(width_sp_tbls_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getInfluenceSpTblSize() const {
    if (influence_sp_tbls_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(influence_sp_tbls_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getParaSpanLenTblSize() const {
    if (para_span_len_sp_tbls_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(para_span_len_sp_tbls_);
        if (array)
            return array->getSize();
...
int RoutingLayerRule::getSpacingSize() const {
    if (spacings_ != 0) {
        ArrayObject<ObjectId> *array = addr<ArrayObject<ObjectId>>(spacings_);
        if (array)
            return array->getSize();
