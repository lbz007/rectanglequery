...
int CutLayerRule::getCutSpacingSize() const {
    if (cut_spacings_ != 0) {
        ArrayObject<ObjectId> *cut_space_array = addr<ArrayObject<ObjectId>>(cut_spacings_);
        if (cut_space_array) {
            return cut_space_array->getSize();
        }
    } 
    return 0;
}
...
void CutLayerRule::addCutSpacing(ObjectId id) {
    ObjectId lib_id = getTechLib()->getId();
    ArrayObject<ObjectId> *vct = nullptr;
    if (cut_spacings_ == 0) {
        vct = Object::createObject<ArrayObject<ObjectId>>(
                kObjectTypeArray, lib_id);
        if (vct == nullptr) return;
        cut_spacings_ = vct->getId();
        vct->setPool(getStorageUtil()->getPool());
        vct->reserve(16);
    } else {
        vct = addr<ArrayObject<ObjectId>>(cut_spacings_);
    }
...
int CutLayerRule::getEnclosureSize() const{
    if (enclosures_ != 0) {
        ArrayObject<ObjectId> *enclosure_array = addr<ArrayObject<ObjectId>>(enclosures_);
        if (enclosure_array) {
            return enclosure_array->getSize();
        }
    }
    return 0;
}
...
int CutLayerRule::getPreferEnclosureSize() const {
    if (prefer_enclosures_ != 0) {
        ArrayObject<ObjectId> *prefer_enclosure_array = addr<ArrayObject<ObjectId>>(prefer_enclosures_);
        if (prefer_enclosure_array) {
            return prefer_enclosure_array->getSize();
        }
    }
    return 0;
}
...
int CutLayerRule::getArraySpacingSize() const {
    if (array_spacings_ != 0) {
        ArrayObject<ObjectId> *array_spacings_array = addr<ArrayObject<ObjectId>>(array_spacings_);
        if (array_spacings_array) {
            return array_spacings_array->getSize();
        }
    }
    return 0;
}
...
SpacingTableOrthogonal *CutLayerRule::getSpTblOrth() const
{
    return (addr<SpacingTableOrthogonal>(sp_tbl_ortho_));
}

void CutLayerRule::setSpTblOrtho(ObjectId id)
{
    sp_tbl_ortho_ = id;
}
...
int ArraySpacing::getArrayCutSize() const { 
    if (array_cuts_ != 0) {
        ArrayObject<Int32> *array_cuts_array = addr<ArrayObject<Int32>>(array_cuts_);
        if (array_cuts_array)
            return array_cuts_array->getSize();
    } 
    return 0;
}
...
void ArraySpacing::addArrayCuts(Int32 array_cuts) {
    ...
    if (array_cuts_ == 0) {
        array_ptr = Object::createObject<ArrayObject<Int32>>(
                      kObjectTypeArray, lib_id);
        if (array_ptr == nullptr) return;
        array_ptr->setPool(getStorageUtil()->getPool());
 ...
 * @return
 */
ArrayObject<Int32> *ArraySpacing::getArraySpacings() const { 
    if (array_spacing_ != 0) {
...
int ArraySpacing::getArraySpacingSize() const { 
    if (array_spacing_ != 0) {
        ArrayObject<Int32> *array_spacing_array = addr<ArrayObject<Int32>>(array_spacing_);
        if (array_spacing_array)
            return array_spacing_array->getSize();
    } 
    return 0;
}
...
/**
 * @brief
 * constructor of SpacingTableOrthogonal
 */
SpacingTableOrthogonal::SpacingTableOrthogonal() {
    num_ortho_ = 0;
    cut_within_ = 0;
    ortho_spacing_ = 0;
}
...
/**
 * @brief
 * destructor of SpacingTableOrthogonal
 */
SpacingTableOrthogonal::~SpacingTableOrthogonal() {
    if (cut_within_ ) {
        cut_within_ = 0;
...
/**
 * @brief
 * get num orthogonal within/spacing
 *
 * @return
 */
Int32 SpacingTableOrthogonal::getNumOrtho() const
{
    return num_ortho_;
}
...
/**
 * @brief
 * set num orthogonal within/spacing
 *
 * @param num
 */
void SpacingTableOrthogonal::setNumOrtho(Int32 num)
{
    num_ortho_ = num;
}
...
/**
 * @brief
 * get orthogonal within value by index
 *
 * @return
 */
Int32 SpacingTableOrthogonal::getCutWithin(Int32 idx) const
{
    if (idx >= num_ortho_) {
        return -1;
    }
    ...
    if (cut_within_ == 0) 
        return -1;
    ArrayObject<Int32> *cut_within__ptr = addr< ArrayObject<Int32> >(cut_within_);
    if (cut_within__ptr == nullptr)
        return -1;

    return (*cut_within__ptr)[idx];
}

/**
 * @brief
 * set orthogonal within value
 *
 * @parama cut_within 
 */
void SpacingTableOrthogonal::setCutWithin(Int32 cut_within)
{
    ObjectId lib_id = getTechLib()->getId();
    ArrayObject<Int32> *cut_within_ptr = nullptr;
    if (cut_within_ == 0) {
        cut_within_ptr = Object::createObject<ArrayObject<Int32>>(
                                         kObjectTypeArray, lib_id);
        if (cut_within_ptr == nullptr) return;
        cut_within_ptr->setPool(getStorageUtil()->getPool());
        cut_within_ptr->reserve(16);        
        cut_within_ = cut_within_ptr->getId();
    } else {
        cut_within_ptr = addr< ArrayObject<Int32> >(cut_within_);
    }

    if (cut_within_ptr) cut_within_ptr->pushBack(cut_within);
}

/**
 * @brief
 * get orthogonal ortho_spacing value by index
 *
 * @return
 */
Int32 SpacingTableOrthogonal::getOrthoSpacing(Int32 idx) const
{
    if (idx >= num_ortho_) {
        return -1;
    }
    ...
    if (ortho_spacing_ == 0) 
        return -1;
    ArrayObject<Int32> *ortho_spacing_ptr = addr< ArrayObject<Int32> >(ortho_spacing_);
    if (ortho_spacing_ptr == nullptr)
        return -1;

    return (*ortho_spacing_ptr)[idx];
}

/**
 * @brief
 * set orthogonal ortho_spacing value
 *
 * @parama ortho_spacing
 */
void SpacingTableOrthogonal::setOrthoSpacing(Int32 ortho_spacing)
{
    ObjectId lib_id = getTechLib()->getId();
    ArrayObject<Int32> *ortho_spacing_ptr = nullptr;
    if (ortho_spacing_ == 0) {
        ortho_spacing_ptr = Object::createObject<ArrayObject<Int32>>(
                                         kObjectTypeArray, lib_id);
        if (ortho_spacing_ptr == nullptr) return;
        ortho_spacing_ptr->setPool(getStorageUtil()->getPool());
        ortho_spacing_ptr->reserve(16);        
        ortho_spacing_ = ortho_spacing_ptr->getId();
    } else {
        ortho_spacing_ptr = addr< ArrayObject<Int32> >(ortho_spacing_);
    }

    if (ortho_spacing_ptr) ortho_spacing_ptr->pushBack(ortho_spacing);
}
