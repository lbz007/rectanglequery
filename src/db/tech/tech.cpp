...
bool Tech::addLayer(Layer *layer) {
    ...
    if (layer_ids_ == 0) {
        array_obj = createObject<ArrayObject<ObjectId>>(kObjectTypeArray,
                                                        this->getId());
 ...
 * @param name
 */
ObjectIndex Tech::getLayerLEFIndexByName(const char *name) {
...
void Tech::addPropertyDefinition(ObjectId pobj_id) {
    ObjectId array_id = property_definitions_array_;
    ArrayObject<ObjectId> *array_ptr = nullptr;

    if (array_id == 0) {
        array_ptr = createObject<ArrayObject<ObjectId>>(kObjectTypeArray,
                                                        this->getId());
        array_ptr->setPool(this->getPool());
        array_ptr->reserve(16);
        ediAssert(array_ptr != nullptr);
        property_definitions_array_ = array_ptr->getId();
    } else {
 ...
 */
ObjectId Tech::getPropertyDefinitionVectorId() {
    //uint32_t index = toInteger<PropType>(type);
    return property_definitions_array_;
}
 ...
 */
ObjectId Tech::getPropertyDefinitionId(const char *prop_name) {
    //ObjectId arr_id = getPropertyDefinitionVectorId(type);
    ArrayObject<ObjectId> *arr = addr<ArrayObject<ObjectId>>(property_definitions_array_);
    if (arr == nullptr || prop_name == nullptr) {
        return 0;
    }
...
PropertyDefinition* Tech::getPropertyDefinition(const char *prop_name) {
    ArrayObject<ObjectId> *arr = addr<ArrayObject<ObjectId>>(property_definitions_array_);
    if (arr == nullptr || prop_name == nullptr) {
        return 0;
    }
 ...
 * @param data
 */
int64_t Tech::micronsToDBU(double data) {
    ...
    return static_cast<int64_t>(dbu_data);
}
 ...
 * @param data
 */
double Tech::dbuToMicrons(int64_t data) {
    Units *db_units_data = getUnits();
    if (db_units_data == nullptr) return -1;
 ...
 * @param data
 */
double Tech::areaDBUToMicrons(int64_t data) {
    Units *db_units_data = getUnits();
    if (db_units_data == nullptr) return -1;
    return data * db_units_data->getMicronPerDBU() *
...
void Tech::addNonDefaultRule(ObjectId ndr_rule_id) {
    ...
    if (!ndr_rules_) {
        arr_ptr = createObject<ArrayObject<ObjectId>>(kObjectTypeArray,
                                                      this->getId());
...
ArrayObject<ObjectId> *Tech::getLayerArray() const {
    ArrayObject<ObjectId> *layer_array =
        addr<ArrayObject<ObjectId>>(layer_ids_);
    return layer_array;
}
/**
...
ObjectId Tech::getNonDefaultRuleIdByName(const char *name) const {
    ArrayObject<ObjectId> *ndr_rule_array =
        addr<ArrayObject<ObjectId>>(ndr_rules_);
...
/**
 * @brief Get the Via Master Index object
 *
 * @param via_master
 * @return int
 */
int Tech::getViaMasterIndex(ViaMaster *via_master) {
    if (via_masters_ == 0) return -1;
    return via_master->getArrayIndex();
}

/**
 * @brief Get the Via Master By Index object
 *
 * @param via_master_index
 * @return ViaMaster*
 */

ViaMaster *Tech::getViaMasterByIndex(int via_master_index) {
    if (via_masters_ == 0) return nullptr;

    ArrayObject<ObjectId> *array_ptr = nullptr;
    ViaMaster *via_master = nullptr;
    array_ptr = addr<ArrayObject<ObjectId>>(via_masters_);
    via_master = addr<ViaMaster>((*array_ptr)[via_master_index]);
...
ViaMaster *Tech::createAndAddViaMaster(std::string &name) {
    ...
    via_master->setArrayIndex(static_cast<uint16_t>(array_ptr->getSize() - 1));
    return via_master;
}

ViaRule *Tech::createViaRule(std::string &name) {
    ViaRule *via_rule =
        createObject<ViaRule>(kObjectTypeViaRule, this->getId());
 ...
 * @return ViaMaster
 */
ViaMaster *Tech::getViaMaster(const std::string &name) {
    SymbolIndex symbol_index = getSymbol(name.c_str());
    if (symbol_index != kInvalidSymbolIndex) {
        ...
        for (auto iter = object_vector.begin(); iter != object_vector.end();
            iter++) {
            ViaMaster *target = addr<ViaMaster>(*iter);
            if (target && (target->getObjectType() == kObjectTypeViaMaster))
                return target;
        }
    }
    return nullptr;
...
ObjectId Tech::getCells() const { return cells_; }
...
void Tech::addCell(ObjectId id) {
    ...
    ObjectId cell_array = getCells();
    if (cell_array == 0) {
        vct = Object::createObject<ArrayObject<ObjectId>>(kObjectTypeArray,
                                                          this->getId());
...
void Tech::setSymbolTable(SymbolTable *stb) {
    storage_util_->setSymbolTable(stb);
}

PolygonTable *Tech::getPolygonTable() {
    return storage_util_->getPolygonTable();
}
void Tech::setPolygonTable(PolygonTable *pt) {
...
/// @brief getSymbol
SymbolIndex Tech::getSymbol(const char *name) {
    SymbolTable *sym_table = getSymbolTable();
    if (sym_table == nullptr) {
        return kInvalidSymbolIndex;
    }
