...
ArrayObject<ObjectId> *ViaRule::getViaRuleMetalLayerArray() const {
    if (metal_layers_ != 0) {
        ArrayObject<ObjectId> *layer_array =
            addr<ArrayObject<ObjectId>>(metal_layers_);
        return layer_array;
    } else {
        return nullptr;
    }
...
ViaRuleCutLayer *ViaRule::getViaRuleCutLayer() {
    ViaRuleCutLayer *cut_layer = addr<ViaRuleCutLayer>(cut_layer_);
    return cut_layer;
}
...
void ViaRule::addViaRuleMetalLayer(ViaRuleMetalLayer *metal_layer) {
    ...
    if (metal_layers_ == 0) {
        metal_layers_ = __createObjectIdArray(16);
    }
    if (metal_layers_) metal_layer_vector = addr<IdArray>(metal_layers_);
...
void ViaRule::addViaMaster(ViaMaster *via_master) {
    ...
    if (via_masters_ == 0) {
        via_masters_ = __createObjectIdArray(16);
    }
    if (via_masters_) via_master_vector = addr<IdArray>(via_masters_);
...
void ViaRule::print() {
    ...
    if (metal_layers_) {
        IdArray *metal_layer_vector = addr<IdArray>(metal_layers_);
        for (IdArray::iterator iter = metal_layer_vector->begin();
             iter != metal_layer_vector->end(); ++iter) {
            ObjectId id = (*iter);
            ViaRuleMetalLayer *metal_layer = addr<ViaRuleMetalLayer>(id);
    ...
    if (via_masters_) {
        IdArray *via_master_vector = addr<IdArray>(via_masters_);
 ...
 * @param is_generate_
 */
void ViaRule::printLEF(IOManager &io_manager) {
    if (!is_generate_) {
        io_manager.write("\nVIARULE %s ", getName().c_str());
    } else {
        io_manager.write("\nVIARULE %s GENERATE", getName().c_str());
        if (is_default_) io_manager.write(" DEFAULT");
    }
    if (metal_layers_) {
        IdArray *metal_layer_vector = addr<IdArray>(metal_layers_);
        for (IdArray::iterator iter = metal_layer_vector->begin();
            ...
            ObjectId id = (*iter);
            ViaRuleMetalLayer *metal_layer = addr<ViaRuleMetalLayer>(id);
            if (metal_layer) metal_layer->printLEF(io_manager, is_generate_);
        }
    }

    if (cut_layer_) {
        ViaRuleCutLayer *cut_layer = addr<ViaRuleCutLayer>(cut_layer_);
        if (cut_layer) cut_layer->printLEF(io_manager);
    }
    ...
    table->getSparsePropertyList<ViaRule>(this, properties);
    if (properties.size() > 0) {
        for(int i = 0; i < properties.size(); i++) {
            Property* prop = table->getSparseProperty<ViaRule>(this, properties[i].c_str());
            prop->printLEF(io_manager);
        }
    }
...
void ViaRuleMetalLayer::printLEF(IOManager &io_manager, int is_generate) {
    Tech *lib = getTechLib();
    Layer *layer = lib->getLayer(layer_id_);
    if (layer)
        io_manager.write("\n     LAYER %s ;", layer->getName());
    ...
    if (!is_generate) {
        ...
        io_manager.write("\n        ENCLOSURE %g %g ;",
            lib->dbuToMicrons(overhang1_), lib->dbuToMicrons(overhang2_));
    }

    if (has_width_) {
        io_manager.write("\n        WIDTH %g TO %g ;",
            lib->dbuToMicrons(min_width_), lib->dbuToMicrons(max_width_));
    }
}
...
void ViaRuleCutLayer::printLEF(IOManager &io_manager) {
    Tech *lib = getTechLib();
    Layer *layer = lib->getLayer(layer_id_);
    if (layer) {
        io_manager.write("\n     LAYER %s ;", layer->getName());
