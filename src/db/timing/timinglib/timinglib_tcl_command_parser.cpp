/**
 * @file timinglib_tcl_command_parser.cpp
 * @date 2020-09-14
 * @brief
 *
    ...
    for (auto tcell : tcells) {
        ...
        for (auto &name : tterm_names) {
            if (term_map.find(name) == term_map.end()) {
                auto term = cell->createTerm(name);
                if (term) {
...
void buildTermMapping() {
    ...
    std::function<void(Tech *, std::unordered_multimap<std::string, Cell *> *,
    ...
    getCells = [](Tech *tech,
                  std::unordered_multimap<std::string, Cell *> *cell_map,
                  std::unordered_set<std::string> *cell_names) {
        uint64_t size = tech->getNumOfCells();
        for (uint64_t index = 0; index < size; ++index) {
            auto c = tech->getCell(index);
            if (c != nullptr) {
    ...
    getCells(tech, &cell_map, &cell_names);
...
int readTimingLibCommand(ClientData cld, Tcl_Interp *itp, int argc,
    ...
    if (argc > 1) {
        ...
        if (lib_file_count != 0) {
            if (bSuccess) {
                // sorting terms and tterms
                buildTermMapping();
                corner->buildTermTTermMap();
                open_edi::util::message->info(
...
int createAnalysisViewCommand(ClientData cld, Tcl_Interp *itp, int argc,
    ...
    if (argc == 7) {
        ...
        AnalysisView *analysis_view = timing_lib->getAnalysisView(args.name);
        if (analysis_view != nullptr) {
            std::string str = "A analysis view with the name ";
            str += args.name;
            open_edi::util::message->issueMsg(
                "TIMINGLIB", 18, open_edi::util::kError, str.c_str());
            return TCL_ERROR;
        }
        ...
        analysis_view = timing_lib->createAnalysisView(args.name);
        if (analysis_view == nullptr) {
            open_edi::util::message->issueMsg(
        ...
        analysis_mode->addAnalysisView(analysis_view);
        analysis_view->setAnalysisMode(analysis_mode->getId());

        analysis_corner->addAnalysisView(analysis_view);
        analysis_view->setAnalysisCorner(analysis_corner->getId());

        analysis_view->createSdc();
...
int createAnalysisModeCommand(ClientData cld, Tcl_Interp *itp, int argc,
    ...
    if (argc == 5) {
        ...
        AnalysisMode *mode = timing_lib->getAnalysisMode(args.name);
        if (mode != nullptr) {
            std::string str = "A analysis mode with the name ";
            str += args.name;
            open_edi::util::message->issueMsg(
                "TIMINGLIB", 18, open_edi::util::kError, str.c_str());
            return TCL_ERROR;
        ...
        mode = timing_lib->createAnalysisMode(args.name);
        if (mode == nullptr) {
            open_edi::util::message->issueMsg("TIMINGLIB", 1,
                                              open_edi::util::kError, "mode",
                                              args.name.c_str());
...
int createAnalysisCornerCommand(ClientData cld, Tcl_Interp *itp, int argc,
    ...
    if (argc >= 7) {
        ...
        AnalysisCorner *corner = timing_lib->getAnalysisCorner(args.name);
        if (corner != nullptr) {
            std::string str = "A analysis corner with the name ";
            str += args.name;
            open_edi::util::message->issueMsg(
                "TIMINGLIB", 18, open_edi::util::kError, str.c_str());
            return TCL_ERROR;
        ...
        corner = timing_lib->createAnalysisCorner(args.name);
        if (corner == nullptr) {
            open_edi::util::message->issueMsg("TIMINGLIB", 1,
                                              open_edi::util::kError, "corner",
                                              args.name.c_str());
...
int readSdcFile(Tcl_Interp *itp, AnalysisView *view) {
    if (!view) {
        message->issueMsg("TIMINGLIB", 23, kError);
        return TCL_ERROR;
    }
    AnalysisMode* mode = view->getAnalysisMode();
    if (!mode) {
        message->issueMsg("TIMINGLIB", 24, kError, (view->getName()).c_str());
        return TCL_ERROR;
    }
    const std::vector<std::string> &sdc_files = mode->getConstraintFiles();
    if (sdc_files.empty()) {
        message->issueMsg("TIMINGLIB", 25, kError, (mode->getName()).c_str());
        return TCL_ERROR;
    }
    std::string tcl_commands;
    getSdcFileContents(tcl_commands, sdc_files, view->getName());
    if (tcl_commands.empty()) {
        message->issueMsg("TIMINGLIB", 26, kError, (mode->getName()).c_str());
        return TCL_ERROR;
    }
    return Tcl_Eval(itp, tcl_commands.c_str());
}

int readMMMCFile(ClientData cld, Tcl_Interp* itp, int argc, const char* argv[]) {
    Command* cmd = CommandManager::parseCommand(argc, argv);
    assert(cmd);
    const std::string &command_line = cmd->getCurrentCmd();
    if (!(cmd->isOptionSet("file"))) {
        message->issueMsg("TIMINGLIB", 19, kError, command_line.c_str());
        return TCL_ERROR;
    }
    std::string file = "";
    bool res = cmd->getOptionValue("file", file);
    if (!res) {
        message->issueMsg("TIMINGLIB", 20, kError, "file", command_line.c_str());
        return TCL_ERROR;
    }
    int status = Tcl_EvalFile(itp, file.c_str());
    if (status == TCL_ERROR) {
        message->issueMsg("TIMINGLIB", 21, kError, file.c_str());
        return TCL_ERROR;
    }

    Timing *timing_db = getTimingLib();
    if (!timing_db) {
        message->issueMsg("TIMINGLIB", 22, kError, command_line.c_str());
        return TCL_ERROR;
    }
    ...
    //TODO build timing graph

    //source sdc
    const uint64_t &view_num =  timing_db->getNumOfAnalysisViews();
    uint64_t index = 0;
    for (; index < view_num; ++index) {
        AnalysisView* view = timing_db->getAnalysisView(index);
        message->info("Reading sdc in view %s\n", (view->getName()).c_str());
        int status = readSdcFile(itp, view);
        if (status == TCL_ERROR) {
            message->issueMsg("TIMINGLIB", 17, kError, (view->getName()).c_str());
        } else {
