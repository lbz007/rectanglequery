...
void TableAxis::clearAllValue() {
    ...
    if (timing_lib != nullptr) {
        ...
        if (object_array != nullptr) object_array->adjustSize(0);
    }
}
void TableAxis::setGroupId(ObjectId id) { group_id_ = id; }

/// get
TableAxisVariable TableAxis::getVariable(void) { return variable_; }
float TableAxis::getValue(TableAxis::IndexType index, float scale /*=1.0f*/) {
    if (values_ == UNINIT_OBJECT_ID) return 0.0f;
    ArrayObject<float>* p = Object::addr<ArrayObject<float>>(values_);
...
float TableAxis::getValueByAxisVariable(float inSlew, float loadCap,
                                        float relatedOutCap) {
    if (variable_ == TableAxisVariable::kInput_Transition_Time ||
        variable_ == TableAxisVariable::kInput_Net_Transition)
        return inSlew;
    else if (variable_ == TableAxisVariable::kTotal_Output_Net_Capacitance)
        return loadCap;
    else if (variable_ ==
             TableAxisVariable::kRelated_Out_Total_Output_Net_Capacitance)
        return relatedOutCap;
    else
        return 0.0f;
}
float TableAxis::getValueScaleByAxisVariable(float inSlewScale,
                                             float loadCapScale,
                                             float relatedOutCapScale) {
    if (variable_ == TableAxisVariable::kInput_Transition_Time ||
        variable_ == TableAxisVariable::kInput_Net_Transition)
        return inSlewScale;
    else if (variable_ == TableAxisVariable::kTotal_Output_Net_Capacitance)
...
ObjectId TableAxis::getGroupId(void) { return group_id_; }

TableAxis::IndexType TableAxis::findAxisIndex(float value) const {
    if (values_ == UNINIT_OBJECT_ID) return 0;
    ArrayObject<float>* p = Object::addr<ArrayObject<float>>(values_);
    if (p == nullptr) return 0;
    IndexType count = p->getSize();
    if (count == 0) return 0;
    IndexType maxIndex = count - 1;
    if (maxIndex == 0 || value <= (*p)[0]) return 0;
    if (value >= (*p)[maxIndex]) return maxIndex - 1;
    int lowerIndex = -1;
    int upperIndex = maxIndex + 1;
    bool ascendOrder = true;
    if ((*p)[0] >= (*p)[maxIndex]) ascendOrder = false;
    while (upperIndex - lowerIndex > 1) {
        int midIndex = (upperIndex + lowerIndex) / 2;
        if (ascendOrder == (value >= (*p)[midIndex]))
            lowerIndex = midIndex;
        else
            upperIndex = midIndex;
    }
...
TableTemplate::~TableTemplate() {
    ...
    if (timing_lib != nullptr) {
        if (axis1_ != UNINIT_OBJECT_ID) {
            ...
            if (t != nullptr) timing_lib->destroyObject<TableAxis>(t);
        }
...
void TableTemplate::setGroupId(ObjectId id) { group_id_ = id; }
...
ObjectId TableTemplate::getGroupId(void) const { return group_id_; }
