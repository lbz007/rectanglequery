...
std::vector<TTerm *> TCell::getTerms(void) {
    std::vector<TTerm *> tterms;
    if (tterms_ == UNINIT_OBJECT_ID) return tterms;

    ArrayObject<ObjectId> *p = Object::addr<ArrayObject<ObjectId>>(tterms_);
    if (p == nullptr) return tterms;

    for (auto iter = p->begin(); iter != p->end(); ++iter) {
        auto term = Object::addr<TTerm>(*iter);
        if (term != nullptr) tterms.emplace_back(term);
    }
    return tterms;
}
...
std::vector<TPgTerm *> TCell::getPgTerms(void) {
    ...
    if (tpg_terms_ == UNINIT_OBJECT_ID) return pg_terms;

    ArrayObject<ObjectId> *p = Object::addr<ArrayObject<ObjectId>>(tpg_terms_);
    if (p == nullptr) return pg_terms;

    for (auto iter = p->begin(); iter != p->end(); ++iter) {
        auto term = Object::addr<TPgTerm>(*iter);
        if (term != nullptr) pg_terms.emplace_back(term);
    }
    return pg_terms;
}

TCell::IndexType TCell::numSequentials() const {
    if (sequentials_ == UNINIT_OBJECT_ID) return 0;

    ArrayObject<ObjectId> *object_vector =
        Object::addr<ArrayObject<ObjectId>>(sequentials_);
    if (object_vector != nullptr)
        return object_vector->getSize();
    else
        return 0;
}
TSequential *TCell::addTSequential() {
    Timing *timing_lib = getTimingLib();
    if (timing_lib) {
        ArrayObject<ObjectId> *p = nullptr;
        if (sequentials_ == UNINIT_OBJECT_ID) {
            p = Object::createObject<ArrayObject<ObjectId>>(
                kObjectTypeArray, timing_lib->getId());
            if (p) {
                sequentials_ = p->getId();
                p->setPool(timing_lib->getPool());
                p->reserve(32);
            }
        } else {
            p = Object::addr<ArrayObject<ObjectId>>(sequentials_);
        }
        if (p != nullptr) {
            auto q = Object::createObject<TSequential>(
                kObjectTypeTimingSequential, timing_lib->getId());
            if (q) {
                q->setGroupId(this->getId());
                ObjectId id = q->getId();
                p->pushBack(id);
            }
            return q;
...
std::vector<TSequential *> TCell::getSequentials(void) {
    std::vector<TSequential *> sequentials;
    if (sequentials_ == UNINIT_OBJECT_ID) return sequentials;

    ArrayObject<ObjectId> *p =
        Object::addr<ArrayObject<ObjectId>>(sequentials_);
    if (p == nullptr) return sequentials;

    for (auto iter = p->begin(); iter != p->end(); ++iter) {
        auto q = Object::addr<TSequential>(*iter);
        if (q != nullptr) sequentials.emplace_back(q);
    }
    return sequentials;
}
...
void TCell::setGroupId(ObjectId id) { group_id_ = id; }
...
ObjectId TCell::getGroupId(void) const { return group_id_; }
...
std::string TCell::getFullName(void) {
    ...
    if (group_id_ != UNINIT_OBJECT_ID) {
        group_obj = Object::addr<Object>(group_id_);
    }
    if (group_obj != nullptr &&
        group_obj->getObjectType() == ObjectType::kObjectTypeTLib) {
        TLib *lib = static_cast<TLib *>(group_obj);
...
void TCell::move(TCell &&rhs) {
    ...
    sequentials_ = std::move(rhs.sequentials_);
    ...
    rhs.group_id_ = UNINIT_OBJECT_ID;
    rhs.term_map_.clear();
...
OStreamBase &operator<<(OStreamBase &os, TCell const &rhs) {
    ...
    os << DataFieldName("sequentials_") << rhs.sequentials_;
    {
        os << rhs.numSequentials();
        os << DataBegin("[");
        if (rhs.sequentials_ != UNINIT_OBJECT_ID) {
            ArrayObject<ObjectId> *object_vector =
                Object::addr<ArrayObject<ObjectId>>(rhs.sequentials_);
            if (object_vector != nullptr) {
                auto delimiter = DataDelimiter("");
                for (int64_t i = 0; i < object_vector->getSize(); ++i) {
                    auto p = Object::addr<TSequential>((*object_vector)[i]);
                    if (p) {
                        os << delimiter << *p;
                        delimiter = DataDelimiter();
                    }
