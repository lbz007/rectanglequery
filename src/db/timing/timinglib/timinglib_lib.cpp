...
void TLib::setGroupId(ObjectId id) { group_id_ = id; }
...
void TLib::addSupplyVoltage(const std::string &name, float f) {
    ...
    if (timing_lib) {
        SymbolIndex index = timing_lib->getOrCreateSymbol(name.c_str());
        if (index != kInvalidSymbolIndex) {
            supply_voltage_map_[index] = f;
            timing_lib->addSymbolReference(index, this->getId());
...
ObjectId TLib::getGroupId(void) const { return group_id_; }
...
void TLib::move(TLib &&rhs) {
    ...
    rhs.operating_conditions_ = UNINIT_OBJECT_ID;
    rhs.wire_loads_ = UNINIT_OBJECT_ID;
    rhs.wire_load_tables_ = UNINIT_OBJECT_ID;
    rhs.wire_load_selections_ = UNINIT_OBJECT_ID;
    rhs.table_templates_ = UNINIT_OBJECT_ID;
