...
void TTerm::setGroupId(ObjectId id) { group_id_ = id; }
...
bool TTerm::isBundle(void) const { return is_bundle_; }
bool TTerm::isBus(void) const { return is_bus_; }
bool TTerm::isDummy(void) { return is_dummy_; }
bool TTerm::isTieHigh(void) {
    if (direction_ == PinDirection::kInout ||
        direction_ == PinDirection::kOutput) {
        TFunction *f = getFunction();
        if (f != nullptr && f->getOp() == FuncOpType::kOP_ONE) {
            return true;
        } else {
            return false;
        }
...
bool TTerm::isTieLow(void) {
    if (getDirection() == PinDirection::kInout ||
        getDirection() == PinDirection::kOutput) {
        TFunction *f = getFunction();
        if (f != nullptr && f->getOp() == FuncOpType::kOP_ZERO) {
            return true;
        } else {
            return false;
        }
...
unsigned int TTerm::busBit(void) const {
    if (!isBus() || !isBundle()) {
        return 0;
    } else {
        std::string pin_name = getName();
        int start = pin_name.find_last_of('[');
        int end = pin_name.find_last_of(']');
        return std::stoi(pin_name.substr(start + 1, end));
    }
}
ObjectId TTerm::getGroupId(void) const { return group_id_; }
...
TimingArc *TTerm::getTimingArc(ObjectId arc_id) {
    if (arc_id != UNINIT_OBJECT_ID) {
        return Object::addr<TimingArc>(arc_id);
    }
    return nullptr;
}
std::vector<TimingArc *> TTerm::getTimingArcs(void) const {
...
TCell *TTerm::getTCell(void) const {
    return Object::addr<TCell>(group_id_);
}
bool TTerm::hasTimingArcWithTTerm(const ObjectId otherTTermId) const {
    TTerm *pTerm = Object::addr<TTerm>(otherTTermId);
    if (pTerm == nullptr) return false;

    std::vector<TimingArc *> timingArcs = getTimingArcs();
    for (auto arc : timingArcs) {
        if (arc->getRelatedPin() == pTerm) return true;
    }

    return false;
}
void TTerm::getTimingArcWithTTerm(const ObjectId otherTTermId,
                                  std::vector<TimingArc *> &arcs) const {
    TTerm *pTerm = Object::addr<TTerm>(otherTTermId);
    if (pTerm == nullptr) return;

    std::unordered_set<TimingArc *> arcSets;
    std::vector<TimingArc *> timingArcs = getTimingArcs();
    for (auto arc : timingArcs) {
        if (arc->getRelatedPin() == pTerm) {
            if (arcSets.find(arc) == arcSets.end()) {
