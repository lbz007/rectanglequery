...
void LibSet::setGroupId(ObjectId id) { group_id_ = id; }
...
TLib *LibSet::getTLib(const std::string &name) const {
    for (const auto &lib_path_to_id : timing_libs_map_) {
        ObjectId id = lib_path_to_id.second;
        auto p = Object::addr<TLib>(id);
...
ObjectId LibSet::getGroupId(void) { return group_id_; }
