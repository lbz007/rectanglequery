...
void LibBuilder::__buildGroupMap(void) {
    group_builder_map_ = {
        ...
        {"ff", BINDG(__buildFf)},
        {"ff_bank", BINDG(__buildFfBank)},
        {"latch", BINDG(__buildLatch)},
        {"latch_bank", BINDG(__buildLatchBank)},
...
void LibBuilder::__buildAttrMap(void) {
    attr_builder_map_ = {
        ...
        // ff
        {"next_state", BINDA(__buildNextState)},
        {"clocked_on", BINDA(__buildClockedOn)},
        {"preset", BINDA(__buildPreset)},
        {"clear", BINDA(__buildClear)},
        {"clear_preset_var1", BINDA(__buildClearPresetVar1)},
        {"clear_preset_var2", BINDA(__buildClearPresetVar2)},

        // latch
        {"data_in", BINDA(__buildDataIn)},
        {"enable", BINDA(__buildEnable)},
    };
...
void LibBuilder::endGroup() {
    if (groupname_stack_.top() == "library") {
        ...
        // Postprocessing output pins and all timing arcs
        // to build TFunction expression tree
        std::vector<tb_namespace::Object *> objects;
        __getObjectsFromTopStack(&objects);
        for (auto &object : objects) {
            if (object->getObjectType() ==
                ...
                std::vector<tb_namespace::TTerm *> lib_pins = cell->getTerms();
                for (tb_namespace::TTerm *pin : lib_pins) {
                    if (pin->getDirection() ==
                            tb_namespace::PinDirection::kInout ||
                        pin->getDirection() ==
                            tb_namespace::PinDirection::kOutput) {
                        tb_namespace::TFunction *f = pin->getFunction();
                        if (f != nullptr) {
                            f->buildExpressionTree(
                                pin, bus_or_bundle_member_pins_map_);
                        }
                        std::vector<tb_namespace::TimingArc *> arcs =
                            pin->getTimingArcs();
                        for (tb_namespace::TimingArc *arc : arcs) {
                            tb_namespace::TFunction *w = arc->getWhen();
                            if (w != nullptr) {
                                w->buildExpressionTree(
                                    pin, bus_or_bundle_member_pins_map_);
                            }
                        }
                    }
                }
                std::vector<tb_namespace::TSequential *> sequentials =
                    cell->getSequentials();
                for (tb_namespace::TSequential *seq : sequentials) {
                    auto p = seq->getClock();
                    if (p)
                        p->buildExpressionTree(nullptr,
                                               bus_or_bundle_member_pins_map_);
                    p = seq->getData();
                    if (p)
                        p->buildExpressionTree(nullptr,
...
void LibBuilder::__buildFf(timinglib_head *h) { __buildFfOrLatch(h, true); }
void LibBuilder::__buildFfBank(timinglib_head *h) {
    __buildFfBankOrLatchBank(h, true);
}
void LibBuilder::__buildLatch(timinglib_head *h) { __buildFfOrLatch(h, false); }
void LibBuilder::__buildLatchBank(timinglib_head *h) {
    __buildFfBankOrLatchBank(h, false);
}
void LibBuilder::__buildFfOrLatch(timinglib_head *h, bool bFf) {
    if (h != nullptr) {
        std::vector<std::string> variables;
        for (auto v = h->list; v; v = v->next) {
            if (__isStringType(v)) variables.emplace_back(v->u.string_val);
        }
        if (variables.size() == 2) {
            GOBJECTS
            for (auto object : objects) {
                if (object->getObjectType() ==
                    ...
                    tb_namespace::Object *obj = nullptr;
                    if (cell) {
                        cell->setSequential(true);
                        auto t1 = cell->getOrCreateTerm(variables[0]);
                        auto t2 = cell->getOrCreateTerm(variables[1]);
                        if (t1 && t2) {
                            tb_namespace::TSequential *p =
                                cell->addTSequential();
                            p->setRegister(bFf);
                            p->setOutput(t1->getId());
                            p->setOutputInv(t2->getId());
                            obj = p;
                        }
                    }
...
void LibBuilder::__buildFfBankOrLatchBank(timinglib_head *h, bool bFfBank) {
    if (h != nullptr) {
        int bits = -1;
        std::vector<std::string> variables;
        for (auto v = h->list; v; v = v->next) {
            if (__isStringType(v))
                variables.emplace_back(v->u.string_val);
            else if (__isIntType(v))
                bits = v->u.int_val;
        }
        if (variables.size() == 2 && bits > 0) {
            GOBJECTS
            ObjectList *seqList = nullptr;
            ObjectList *l = nullptr;
            for (auto object : objects) {
                if (object->getObjectType() ==
                    tb_namespace::ObjectType::kObjectTypeTCell) {
                    ...
                    if (cell) {
                        ...
                        for (int i = 1; i <= bits; i++) {
                            std::string var1 = variables[0];
                            if (bits != 1) var1 += std::to_string(i);
                            std::string var2 = variables[1];
                            if (bits != 1) var2 += std::to_string(i);
                            auto t1 = cell->getOrCreateTerm(var1);
                            auto t2 = cell->getOrCreateTerm(var2);
                            if (t1 && t2) {
                                tb_namespace::TSequential *p =
                                    cell->addTSequential();
                                p->setRegister(bFfBank);
                                p->setOutput(t1->getId());
                                p->setOutputInv(t2->getId());

                                if (seqList == nullptr) {
                                    seqList = new ObjectList();
                                    seqList->object = p;
                                    l = seqList;
                                } else {
                                    l = new ObjectList();
            ...
            if (seqList != nullptr) {
                if (!objects_stack_.empty()) {
                    for (ObjectList *v = seqList; v; v = v->next) {
                        v->owner = objects_stack_.top();
                    }
                }
                objects_stack_.push(seqList);
                push_status_ = 1;
            }
        }
    }
...
void LibBuilder::__buildTiming(timinglib_head *h) {
    ...
    for (auto &p : libpins) {
        auto t = tb_namespace::Object::createObject<tb_namespace::TimingArc>(
            ...
            timing_lib->getId());
        if (t) {
            tb_namespace::TimingArcData *arc_data = t->createData();
            if (arc_data == nullptr) {
                continue;
            }
            t->setGroupId(p->getId());
...
void LibBuilder::__buildCellRise(timinglib_head *h) {
    ...
    if (v && __isStringType(v)) {
        ...
        tb_namespace::ObjectId owner = cell->getGroupId();
        tb_namespace::TLib *lib =
        ...
        if (axis1) {
            axis_[0] =
                tb_namespace::Object::createObject<tb_namespace::TableAxis>(
                    ...
                    timing_lib->getId());
            if (axis_[0]) {
                axis_[0]->setVariable(axis1->getVariable());
                tb_namespace::ArrayObject<float> *pData = axis1->getValues();
                if (pData) {
                    for (tb_namespace::Object::IndexType i = 0;
                         i < pData->getSize(); ++i) {
                        axis_[0]->addValue((*pData)[i]);
                    }
                }
            }
        } else {
        ...
        if (axis2) {
            axis_[1] =
                tb_namespace::Object::createObject<tb_namespace::TableAxis>(
                    ...
                    timing_lib->getId());
            if (axis_[1]) {
                axis_[1]->setVariable(axis2->getVariable());
                tb_namespace::ArrayObject<float> *pData = axis2->getValues();
                if (pData) {
                    for (tb_namespace::Object::IndexType i = 0;
                         i < pData->getSize(); ++i) {
                        axis_[1]->addValue((*pData)[i]);
                    }
                }
            }
        } else {
        ...
        if (axis3) {
            axis_[2] =
                tb_namespace::Object::createObject<tb_namespace::TableAxis>(
                    ...
                    timing_lib->getId());
            if (axis_[2]) {
                axis_[2]->setVariable(axis3->getVariable());
                tb_namespace::ArrayObject<float> *pData = axis3->getValues();
                if (pData) {
                    for (tb_namespace::Object::IndexType i = 0;
                         i < pData->getSize(); ++i) {
                        axis_[2]->addValue((*pData)[i]);
                    }
                }
            }
        } else {
    ...
    for (auto object : objects) {
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                unit.suffix = suffix;
                unit.scale =
                    d * tb_namespace::UnitMultiply::getTimeUnitMultiply(
                            unit.suffix);
            }
        }
    }
    ...
    for (auto object : objects) {
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                unit.suffix = suffix;
                unit.scale =
                    d * tb_namespace::UnitMultiply::getResistanceUnitMultiply(
                            unit.suffix);
            }
        }
    }
        ...
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                if (v->next != nullptr && __isStringType(v->next)) {
                    ...
                    unit.scale =
                        d *
                        tb_namespace::UnitMultiply::getCapacitiveUnitMultiply(
                            unit.suffix);
                }
            }
        }
    ...
    for (auto object : objects) {
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                unit.suffix = suffix;
                unit.scale =
                    d * tb_namespace::UnitMultiply::getVoltageUnitMultiply(
                            unit.suffix);
            }
        }
    }
    ...
    for (auto object : objects) {
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                unit.suffix = suffix;
                unit.scale =
                    d * tb_namespace::UnitMultiply::getCurrentUnitMultiply(
                            unit.suffix);
            }
        }
    }
    ...
    for (auto object : objects) {
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                unit.suffix = suffix;
                unit.scale =
                    d * tb_namespace::UnitMultiply::getPowerUnitMultiply(
                            unit.suffix);
            }
        }
    }
    ...
    for (auto object : objects) {
        if (object->getObjectType() ==
            ...
            if (units != nullptr) {
                ...
                unit.suffix = suffix;
                unit.scale =
                    d * tb_namespace::UnitMultiply::getDistanceUnitMultiply(
                            unit.suffix);
            }
        }
    }
...
void LibBuilder::__buildRelatedGroundPin(buildParam) {
    if (cur_bus_or_bundle_name_ != "" && __isStringType(v)) {
        ...
        if (findIt != bus_or_bundle_member_pins_map_.end()) {
            auto &vec = findIt->second;
            for (auto &term : vec) {
                tb_namespace::ObjectId owner = term->getGroupId();
    ...
    if (__isStringType(v)) {
        for (auto term : libpins) {
            if (term) {
                tb_namespace::ObjectId ownerId = term->getGroupId();
...
void LibBuilder::__buildRelatedPin(buildParam) {
    ...
    for (auto &p : timingarcs) {
        tb_namespace::ObjectId owner = p->getGroupId();
        if (owner != UNINIT_OBJECT_ID) {
            auto term = tb_namespace::Object::addr<tb_namespace::TTerm>(owner);
            if (term) terms_owner.emplace_back(term);
        }
    }
    if (terms_owner.empty()) return;
    tb_namespace::TTerm *term_owner = terms_owner[0];
    assert(term_owner);
    if (term_owner == nullptr) return;

    tb_namespace::TCell *cell_owner = nullptr;
    tb_namespace::ObjectId cell_owner_object_id = term_owner->getGroupId();
    if (cell_owner_object_id != UNINIT_OBJECT_ID)
        cell_owner = tb_namespace::Object::addr<tb_namespace::TCell>(
            cell_owner_object_id);
    if (cell_owner == nullptr) return;

    size_t term_size = terms_owner.size();
    std::vector<std::pair<tb_namespace::TimingArc *, tb_namespace::TTerm *> >
        arc_related_terms;
        ...
        if (findIt != bus_or_bundle_member_pins_map_.end()) {
            ...
            if (term_size == vec.size()) {
                for (int i = 0; i < term_size; i++) {
                    arc_related_terms.emplace_back(timingarcs[i], vec[i]);
                }
            } else {
            ...
            auto term = cell_owner->getOrCreateTerm(s);
            if (term) related_terms.emplace_back(term);
        }
    }
    for (auto &term : related_terms) {
        for (auto &arc : timingarcs) {
            arc_related_terms.emplace_back(arc, term);
        }
    }
    if (arc_related_terms.empty()) return;

    size_t count = 0;
    size_t exist_arc_size = timingarcs.size();
    for (auto &arc_term : arc_related_terms) {
        count++;
        tb_namespace::TimingArc *arc = arc_term.first;
        tb_namespace::TTerm *related_term = arc_term.second;
        if ((arc == nullptr) || (related_term == nullptr)) {
            continue;
        }
        auto related_term_id = related_term->getId();
        if (count <= exist_arc_size) {
            arc->setRelatedPin(related_term_id);
        } else {
            tb_namespace::Timing *timing_lib = tb_namespace::getTimingLib();
            auto new_arc =
                tb_namespace::Object::createObject<tb_namespace::TimingArc>(
                    tb_namespace::ObjectType::kObjectTypeTimingArc,
                    timing_lib->getId());
            if (new_arc == nullptr) continue;
            auto new_arc_object_id = new_arc->getId();
            *new_arc = *arc;
            new_arc->setId(new_arc_object_id);
            new_arc->setRelatedPin(related_term_id);
            tb_namespace::ObjectId owner = arc->getGroupId();
            auto term = tb_namespace::Object::addr<tb_namespace::TTerm>(owner);
            if (term) {
                term->addTimingarc(new_arc_object_id);
            }
        }
    }
}
        ...
        if (obj->getObjectType() ==
            ...
            for (auto p = v; p; p = p->next) {
                if (__isIntType(p) || __isDoubleType(p)) {
                   ...
                   tb_namespace::ObjectType::kObjectTypeTimingArc) {
            if (axis_[0]) {
                axis_[0]->clearAllValue();
                for (auto p = v; p; p = p->next) {
                    if (__isIntType(p) || __isDoubleType(p)) {
                        axis_[0]->addValue(__getDoubleValue(p));
                    } else if (__isStringType(p)) {
                        std::vector<double> values;
                        __getStringFloatList(p->u.string_val, &values);
                        for (auto f : values) axis_[0]->addValue(f);
                    }
                }
            }
        }
        ...
        if (obj->getObjectType() ==
            ...
            for (auto p = v; p; p = p->next) {
                if ((__isIntType(p) || __isDoubleType(p))) {
                   ...
                   tb_namespace::ObjectType::kObjectTypeTimingArc) {
            if (axis_[1]) {
                axis_[1]->clearAllValue();
                for (auto p = v; p; p = p->next) {
                    if (__isIntType(p) || __isDoubleType(p)) {
                        axis_[1]->addValue(__getDoubleValue(p));
                    } else if (__isStringType(p)) {
                        std::vector<double> values;
                        __getStringFloatList(p->u.string_val, &values);
                        for (auto f : values) axis_[1]->addValue(f);
                    }
                }
            }
        }
        ...
        if (obj->getObjectType() ==
            ...
            for (auto p = v; p; p = p->next) {
                if ((__isIntType(p) || __isDoubleType(p))) {
                   ...
                   tb_namespace::ObjectType::kObjectTypeTimingArc) {
            if (axis_[3]) {
                axis_[3]->clearAllValue();
                for (auto p = v; p; p = p->next) {
                    if (__isIntType(p) || __isDoubleType(p)) {
                        axis_[3]->addValue(__getDoubleValue(p));
                    } else if (__isStringType(p)) {
                        std::vector<double> values;
                        __getStringFloatList(p->u.string_val, &values);
                        for (auto f : values) axis_[3]->addValue(f);
                    }
                }
            }
        }
        ...
        if (p->getObjectType() ==
            ...
            if (axis_[0] && axis_[1] && axis_[2]) {
                auto p = tb_namespace::Object::createObject<
                    ...
                    timing_lib->getId());
                if (p) {
                    axis_[0]->setGroupId(p->getId());
                    p->setAxis1(axis_[0]->getId());
                    axis_[1]->setGroupId(p->getId());
                    p->setAxis2(axis_[1]->getId());
                    axis_[2]->setGroupId(p->getId());
...
void LibBuilder::__buildNextState(buildParam) {
    if (v && __isStringType(v)) {
        std::vector<tb_namespace::TSequential *> sequentials;
        __getSequentialsFromTopStack(&sequentials);
        for (auto &p : sequentials) {
            p->setData(v->u.string_val);
        }
    }
}
void LibBuilder::__buildClockedOn(buildParam) {
    if (v && __isStringType(v)) {
        std::vector<tb_namespace::TSequential *> sequentials;
        __getSequentialsFromTopStack(&sequentials);
        for (auto &p : sequentials) {
            p->setClock(v->u.string_val);
        }
    }
}
void LibBuilder::__buildPreset(buildParam) {
    if (v && __isStringType(v)) {
        std::vector<tb_namespace::TSequential *> sequentials;
        __getSequentialsFromTopStack(&sequentials);
        for (auto &p : sequentials) {
            p->setPreset(v->u.string_val);
        }
    }
}
void LibBuilder::__buildClear(buildParam) {
    if (v && __isStringType(v)) {
        std::vector<tb_namespace::TSequential *> sequentials;
        __getSequentialsFromTopStack(&sequentials);
        for (auto &p : sequentials) {
            p->setClear(v->u.string_val);
        }
    }
}
void LibBuilder::__buildClearPresetVar1(buildParam) {
    if (v && __isStringType(v)) {
        std::vector<tb_namespace::TSequential *> sequentials;
        __getSequentialsFromTopStack(&sequentials);
        for (auto &p : sequentials) {
            if (!strcmp(v->u.string_val, "L"))
                p->setClearPresetOut(tb_namespace::TimingClearPreset::kL);
            else if (!strcmp(v->u.string_val, "H"))
                p->setClearPresetOut(tb_namespace::TimingClearPreset::kH);
            else if (!strcmp(v->u.string_val, "N"))
                p->setClearPresetOut(tb_namespace::TimingClearPreset::kN);
            else if (!strcmp(v->u.string_val, "T"))
                p->setClearPresetOut(tb_namespace::TimingClearPreset::kT);
            else if (!strcmp(v->u.string_val, "X"))
                p->setClearPresetOut(tb_namespace::TimingClearPreset::kX);
        }
    }
}
void LibBuilder::__buildClearPresetVar2(buildParam) {
    if (v && __isStringType(v)) {
        std::vector<tb_namespace::TSequential *> sequentials;
        __getSequentialsFromTopStack(&sequentials);
        for (auto &p : sequentials) {
            if (!strcmp(v->u.string_val, "L"))
                p->setClearPresetOutInv(tb_namespace::TimingClearPreset::kL);
            else if (!strcmp(v->u.string_val, "H"))
                p->setClearPresetOutInv(tb_namespace::TimingClearPreset::kH);
            else if (!strcmp(v->u.string_val, "N"))
                p->setClearPresetOutInv(tb_namespace::TimingClearPreset::kN);
            else if (!strcmp(v->u.string_val, "T"))
                p->setClearPresetOutInv(tb_namespace::TimingClearPreset::kT);
            else if (!strcmp(v->u.string_val, "X"))
                p->setClearPresetOutInv(tb_namespace::TimingClearPreset::kX);
        }
    }
}

void LibBuilder::__buildDataIn(buildParam) { __buildNextState(name, v); }
void LibBuilder::__buildEnable(buildParam) { __buildClockedOn(name, v); }
...
void LibBuilder::__getSequentialsFromTopStack(
    std::vector<tb_namespace::TSequential *> *libTSequentials) {
    GOBJECTS
    for (auto object : objects) {
        if (object->getObjectType() ==
            tb_namespace::ObjectType::kObjectTypeTimingSequential) {
            libTSequentials->emplace_back(
                static_cast<tb_namespace::TSequential *>(object));
        }
    }
}
...
void LibBuilder::__getPinNamesFromBusRange(const char *str,
    ...
    if (error == 0 && compiled.re_nsub >= 2) {
        ...
        if (from == -1) {
            ...
            std::string str = bus_name + std::string("[") +
                              std::to_string(from) + std::string("]");
            pinNames->emplace_back(str);
        } else {
            if (from > to) {
                from = from + to;
...
tb_namespace::TCell *LibBuilder::__getTcellByTimingarc(
    ...
    tb_namespace::ObjectId owner = t->getGroupId();
