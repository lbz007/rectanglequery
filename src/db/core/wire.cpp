 ...
 *
 */
void WirePatch::printDEF(IOManager &io_manager) {
    Tech* lib = getTopCell()->getTechLib();
    Layer* layer = nullptr;
    layer = lib->getLayer(getLayerNum());

    io_manager.write(
        "    NEW %s ( %d %d ) RECT ( %d %d %d %d ) \n",
        layer->getName(), loc_x_, loc_y_,
        delta_x_1_, delta_y_1_, delta_x_2_, delta_y_2_);
}
...
void Wire::setNet(ObjectId net) { net_ = net; }
void Wire::setNet(Net* net) {
    if (net) net_ = net->getId();
}

void Wire::setNet(SpecialNet* net) {
    if (net) net_ = net->getId();
}
...
SpecialNet* Wire::getSpecialNet() {
    if (net_) {
        return addr<SpecialNet>(net_);
    } else {
        return nullptr;
    }
}

Box Wire::getBBox() const {
    Box box(x_, y_, x_ + length_, y_ + height_);
    return box;
}
Layer* Wire::getLayer() {
    Tech* lib = getTopCell()->getTechLib();
...
void Wire::printDEF(IOManager &io_manager) {
    Tech* lib = getTopCell()->getTechLib();
    Layer* layer = nullptr;
    layer = lib->getLayer(getLayerNum());
    int status = 0;
    ...
    if (status) {
        switch (status) {
            ...
            case 2:
                ...
                break;
            case 3:
                io_manager.write("  + ROUTED \n    ");
                break;
            case 4:
                io_manager.write("  + NOSHIELD \n    ");
                break;
