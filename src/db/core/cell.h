...
class HierData : public Object {
  public:
    HierData() { __init(); }
    HierData(HierData const &hier_data) { __copy(hier_data); }
    ~HierData() {}
    ...
    // Hier owner index:
    ObjectIndex getHierOwnerIndex() const;
    void setHierOwnerIndex(ObjectIndex hier_owner_index);
  protected:
    void __copy(HierData const &hier_data);
  ...
  private:
    ...
    ObjectIndex hier_owner_index_; /// using 32bits to store cell index.
};

//HierData with flatten instances.
class FHierData : public HierData {
  public:
    FHierData() : HierData() { __init(); }
    FHierData(HierData const &hier_data) : 
      HierData(hier_data) { __copy(hier_data); }

    ~FHierData() {};

    ArrayObject<ObjectId> *getFlattenInstArray() const;
    ArrayObject<ObjectId> *getOrCreateFlattenInstArray() const;

    bool insertFlattenInst(ObjectId id);
    bool copy(HierData const *hier_data);
  private:
    void __init();
    void __copy(HierData const &hier_data);

    ObjectId flatten_insts_; /// flatten inst array;
};


/// @brief stores access group info on cell
class AccessGroup : public Object {
  public:
    AccessGroup();
    ~AccessGroup();

    unsigned char getSet() const;
    Orient        getOrient() const;
    int           getCost() const;
    Cell*         getCell() const;

    std::vector<std::pair<Term*, PinAccess*>> getAccessSet(); // or just provide an iterator

    void setOrient(Orient o);
    void setCell(Cell* cell);
    void setSet(unsigned char s);
    void setCost(int cost);
    void setAccessSet(std::vector<std::pair<Term*, PinAccess*>>&& vec);

  private:
    ObjectId                                 cell_;    // cell it belongs to
    std::vector<std::pair<Term*, PinAccess*>> acc_vec_; // DRC clean access point set

    int           cost_; // cost
    unsigned char ori_;  // instance orientation
    unsigned char set_;  // reserved
};
...
class Cell : public Object {
  public:
    ...
    SymbolIndex getSymbol(const char *name);
    bool addSymbolReference(SymbolIndex index, ObjectId owner);
    void setPool(MemPagePool *p);
    MemPagePool *getPool();
    ...
    ArrayObject<ObjectId> *getFInstanceArray() const;
    ...
    void addFlattenInstance(ObjectId id);
    ...
    Term *createTerm(const std::string &name);
    Bus *createBus(std::string &name);
    Inst *createInstance(std::string const &name);
    Inst *createInstance(std::string const &name, std::string const &master_name);
    Inst *createFlattenInstance(std::string const &name, std::string const &master_name);
    ...
    bool isClassNone() { return class_type_ == kNoClass? true : false;}
    ...
    //Operation:
    void flatten(Inst *hinst, Inst **finst);
    bool isFlattenInstId(ObjectId id);
    // Print:
    // void print();
    void printLEF(IOManager &io_manager);

    //access group
    const std::vector<AccessGroup>* getPinAccessGroups(unsigned char orient, unsigned char set) const;

    bool appendPinAccessGroup(unsigned char orient, unsigned char set, AccessGroup& PAG);
    void resetPAG();
    bool isPAGCreated() { return (bool)is_PAG_set_; };

    AccessGroup* getFirstPinAccessGroup(unsigned char orient, unsigned char set);

  private:
    void __init();
    const HierData *__getConstHierData() const;
    HierData *__getHierData();
    FHierData *__getFHierData() const;
    FHierData *__getOrCreateFHierData();
    Inst *__createInstance(
              std::string const &name, bool is_flatten);
    Inst *__createInstance(
              std::string const &name, std::string const &master_name, bool is_flatten);
    ...
    //access group
    std::vector<AccessGroup> access_groups_[kMaxOrientationNum][kMaxSetNum];
