...
int nonDefaultRuleTest(ClientData cld, Tcl_Interp *itp, int argc,
    ...
    testObj = getTopCell()->createObject<NonDefaultRule>(kObjectTypeNonDefaultRule);
    ...
    if (top_cell) {
        ...
        table->getSparsePropertyList<NonDefaultRule>(testObj, properties);
        if (properties.size() > 0) {
            for(int i = 0; i < properties.size(); i++) {
                Property* prop = table->getSparseProperty<NonDefaultRule>(testObj, properties.at(i).c_str());
                if (prop != nullptr) {
                    os << *prop << endl_c;
                }
            }
