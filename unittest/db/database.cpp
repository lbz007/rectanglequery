...
TEST_F(DatabaseTest, dbInit) {
  open_edi::util::setAppPath("");  
  open_edi::util::utilInit();
  open_edi::util::MemPool::initMemPool();
  open_edi::db::initTopCell();
...
TEST_F(DatabaseTest, Property) {
  Cell* top_cell = db::getTopCell();
  ...
  prop_definition->setPropName("pa");
  prop_definition->setDataType(PropDataType::kInt);
  ObjectId obj_id = prop_definition->getId();
  getTopCell()->getTechLib()->addPropertyDefinition(obj_id);

  Tech *lib = top_cell->getTechLib();
  Layer *layer = Object::createObject<Layer>(kObjectTypeLayer, lib->getId());
  lib->addLayer(layer);  
  std::vector<std::string> props;
  pm->getSparsePropertyList<Layer>(layer, props);
  EXPECT_EQ(props.size(), 0);

  pm->setProperty<Layer>(layer, "pa", 10);
  pm->getSparsePropertyList<Layer>(layer, props);
  EXPECT_EQ(props.size(), 1);
  EXPECT_EQ(pm->getPropertyInt<Layer>(layer, "pa"), 10);
}
